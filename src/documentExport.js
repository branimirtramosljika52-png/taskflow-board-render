...
function normalizePdfSignatureAppearanceSettings(value = null) {
  ...
  return {
    ...
    showLogo: normalizePdfSignatureBoolean(raw.showLogo ?? raw.logoEnabled ?? raw.logo, DEFAULT_PDF_SIGNATURE_APPEARANCE_SETTINGS.showLogo),
    ...
    border: normalizePdfSignatureBoolean(raw.border ?? raw.borderEnabled ?? raw.showBorder, DEFAULT_PDF_SIGNATURE_APPEARANCE_SETTINGS.border),
...
function normalizePdfSignatureFieldSpec(item = {}, options = {}) {
  ...
  return {
    ...
    roleLabel: clean(item.roleLabel || item.role || item.signatureLabel) || signatureFieldRole,
...
function normalizeDocxSpecialPlaceholderValue(value) {
  ...
  if (blockType === "rich_text") {
    const html = sanitizeRichTextHtml(value.html || value.richHtml || value.value || "");
    return html
      ? {
        type: "rich_text",
        html,
...
function buildWordRichTextXml(value = {}) {
  const text = richTextHtmlToPlainText(value.html || value.richHtml || value.text || "");
  const lines = String(text || "").replace(/\r\n/g, "\n").split("\n");
  const paragraphs = lines.length > 0 ? lines : [""];
  return paragraphs.map((line) => {
    const trimmed = clean(line);
    const isListItem = /^[-•]\s+/.test(trimmed) || /^\d+[.)]\s+/.test(trimmed);
    const safeLine = isListItem ? trimmed.replace(/^[-•]\s+/, "• ").replace(/^\d+[.)]\s+/, "• ") : trimmed;
    return buildWordParagraphXml(safeLine, {
      spacingAfter: isListItem ? 30 : 60,
    });
  }).join("");
}
...
function buildHtmlTemplateRichTextPlaceholder(value = {}) {
  const html = sanitizeRichTextHtml(value.html || value.richHtml || value.value || "");
...
function normalizePdfSignatureAnchorText(value = "") {
  return clean(value)
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
...
function getPdfSignatureAnchorKey(anchor = {}) {
  return [
    ...
    Math.round((Number(anchor.x) || 0) * 10),
    Math.round((Number(anchor.y) || 0) * 10),
    normalizePdfSignatureAnchorDigits(anchor.str || ""),
  ].join(":");
}

function buildPdfSignatureAnchorContext(page = {}, anchor = {}) {
  const anchorY = Number(anchor?.y);
  const nearbyLines = (Array.isArray(page.lines) ? page.lines : [])
    .filter((line) => !Number.isFinite(anchorY) || Math.abs(Number(line.y) - anchorY) <= 46)
    .sort((left, right) => right.y - left.y)
    .map((line) => line.str);
  return normalizePdfSignatureAnchorText(nearbyLines.join(" "));
}

function getPdfSignatureRoleNeedles(field = {}) {
  const roleText = normalizePdfSignatureAnchorText([
    field.roleLabel,
    field.role,
    field.label,
  ].filter(Boolean).join(" "));
  const needles = [];
  if (roleText) {
    needles.push(roleText);
  }
  if (/\bispitiv/.test(roleText)) {
    needles.push("ispitiv");
  }
  if (/\bodgovorn|\bnositelj/.test(roleText)) {
    needles.push("odgovorn", "nositelj");
  }
  return Array.from(new Set(needles.filter(Boolean)));
}

function collectPdfSignatureOibAnchorCandidates(textAnchors = [], signatureFieldOib = "") {
  ...
  for (let pageIndex = textAnchors.length - 1; pageIndex >= 0; pageIndex -= 1) {
    ...
    candidates.push(...pageCandidates.sort((left, right) => left.y - right.y));
  }
  return candidates;
}
...
function findPdfSignatureOibAnchor(textAnchors = [], fieldOrOib = "", options = {}) {
  const field = fieldOrOib && typeof fieldOrOib === "object"
    ? fieldOrOib
    : { signatureFieldOib: fieldOrOib };
  const candidates = collectPdfSignatureOibAnchorCandidates(textAnchors, field.signatureFieldOib);
  if (candidates.length === 0) {
    return null;
  }
  const usedAnchorKeys = options.usedAnchorKeys instanceof Set ? options.usedAnchorKeys : new Set();
  const roleNeedles = getPdfSignatureRoleNeedles(field);
  const scored = candidates.map((candidate, index) => {
    const anchorKey = getPdfSignatureAnchorKey(candidate);
    const unused = !usedAnchorKeys.has(anchorKey);
    const exactRoleMatch = roleNeedles[0] && candidate.contextText.includes(roleNeedles[0]);
    const roleMatch = roleNeedles.some((needle) => candidate.contextText.includes(needle));
    return {
      candidate: {
        ...candidate,
        anchorKey,
      ...
      index,
      score: (exactRoleMatch ? 120 : 0)
        + (roleMatch ? 80 : 0)
        + (unused ? 20 : 0)
        + (candidate.source === "oib_line" ? 4 : 0),
    };
  }).sort((left, right) => (
    right.score - left.score
    || left.index - right.index
...
export async function addPdfSignatureFieldsToBuffer(pdfBuffer = Buffer.alloc(0), signatureFields = [], options = {}) {
  ...
  normalizedFields.forEach((field, index) => {
    ...
    if (canUseAnchorPage && oibAnchor?.anchorKey) {
      usedTextAnchorKeys.add(oibAnchor.anchorKey);
    }
...
function isOfferPdfIncludedServiceItem(item = {}) {
...
function formatOfferPdfItemAmount(item = {}, currency = "EUR") {
  if (isOfferPdfIncludedServiceItem(item)) {
    return "Uključeno";
  }
...
function getOfferPdfTaxLabel(offer = {}) {
  const taxRate = clean(offer?.taxRate);
  return taxRate ? `PDV (${taxRate}%)` : "PDV";
}

function getOfferPdfFinancialRows(offer = {}, currency = "EUR") {
  const hasDiscount = Number(offer.discountRate ?? 0) > 0 || Number(offer.discountTotal ?? 0) > 0;
  const rows = [
    {
      id: "subtotal",
      label: "Međuzbroj",
      amount: formatOfferPdfCurrency(offer.subtotal ?? 0, currency),
    },
  ];
  ...
  if (hasDiscount) {
    rows.push(
      {
        id: "discount",
        label: Number(offer.discountRate ?? 0) > 0 ? `Rabat (${offer.discountRate}%)` : "Rabat",
        amount: formatOfferPdfCurrency(offer.discountTotal ?? 0, currency),
      },
      {
        id: "taxable-subtotal",
        label: "Osnovica za PDV",
        amount: formatOfferPdfCurrency(offer.taxableSubtotal ?? 0, currency),
      },
    );
  }
  ...
  rows.push(
    {
      id: "tax",
      label: getOfferPdfTaxLabel(offer),
      amount: formatOfferPdfCurrency(offer.taxTotal ?? 0, currency),
    },
    {
      id: "total",
      label: "Ukupno s PDV-om",
      amount: formatOfferPdfCurrency(offer.total ?? 0, currency),
      isGrand: true,
    },
  );
...
function buildOfferHtmlFinancialFooterRows(offer = {}, currency = "EUR", { colspan = 5 } = {}) {
  return getOfferPdfFinancialRows(offer, currency).map((row) => `
    <tr class="${row.isGrand ? "offer-html-total-row" : "offer-html-subtotal-row"}">
      <td colspan="${colspan}">${escapeOfferHtml(row.label)}</td>
      <td>${escapeOfferHtml(row.amount)}</td>
    </tr>
  `).join("");
}

function buildOfferHtmlPlanFinancialSummary(offer = {}, currency = "EUR") {
  if (offer.showTotalAmount === false) {
    return "";
  }

  return `
    <div class="offer-html-plan-summary">
      ${getOfferPdfFinancialRows(offer, currency).map((row) => `
        <div class="${row.isGrand ? "is-grand" : ""}">
          <span>${escapeOfferHtml(row.label)}</span>
          <strong>${escapeOfferHtml(row.amount)}</strong>
        </div>
      `).join("")}
    </div>
  `;
...
function buildOfferHtmlItemsTable(offer = {}, currency = "EUR") {
  ...
  if (planType === "Fixed Plan") {
    const fixedBillableItems = items.filter((item) => !isOfferPdfIncludedServiceItem(item));
    const fixedIncludedItems = items.filter((item) => isOfferPdfIncludedServiceItem(item));
    const fixedBillableRowsHtml = fixedBillableItems.map((item, index) => `
      <tr class="is-billable">
        <td>${index + 1}</td>
        <td>${formatOfferHtmlText(item.description || item.serviceCode || "Stavka")}</td>
        <td>${formatOfferHtmlText(item.unit || "")}</td>
        <td>${formatOfferHtmlText(item.quantity ?? "")}</td>
        <td><span class="offer-html-money">${escapeOfferHtml(formatOfferPdfCurrency(item.unitPrice ?? 0, currency))}</span></td>
        <td><span class="offer-html-money">${escapeOfferHtml(formatOfferPdfItemAmount(item, currency) || formatOfferPdfCurrency(item.totalPrice ?? 0, currency))}</span></td>
      </tr>
    `).join("");
    const fixedIncludedRowsHtml = fixedIncludedItems.map((item, index) => `
      <tr>
        <td>${index + 1}</td>
        <td>${formatOfferHtmlText(item.serviceCode || "")}</td>
        <td>${formatOfferHtmlText(item.description || item.serviceCode || "Usluga")}</td>
      </tr>
    `).join("");

    return `
      <div class="offer-html-fixed-plan-head">
        <strong>Fixed plan</strong>
        <span>${fixedBillableItems.length} naplativo · ${fixedIncludedItems.length} uključeno</span>
      </div>
      ${buildOfferHtmlPlanFinancialSummary(offer, currency)}
      <section class="offer-html-plan-section">
        <h4>Fiksne stavke ponude</h4>
        <table class="offer-html-items-table offer-html-fixed-plan-table">
          <thead>
            <tr>
              <th>#</th>
              <th>Opis</th>
          ...
          <tbody>${fixedBillableRowsHtml || `<tr><td colspan="6">Nema dodanih fiksnih stavki.</td></tr>`}</tbody>
          ${offer.showTotalAmount !== false ? `<tfoot>${buildOfferHtmlFinancialFooterRows(offer, currency, { colspan: 5 })}</tfoot>` : ""}
        </table>
      </section>
      <section class="offer-html-plan-section offer-html-included-services-section">
        <h4>Uključene usluge</h4>
        <table class="offer-html-items-table offer-html-included-services-table">
          <thead>
            <tr>
              <th>#</th>
              <th>Šifra</th>
              <th>Usluga uključena u paušal</th>
            </tr>
          </thead>
          <tbody>${fixedIncludedRowsHtml || `<tr><td colspan="3">Nema dodanih uključenih usluga.</td></tr>`}</tbody>
        </table>
      </section>
    `;

    const billableCount = items.filter((item) => !isOfferPdfIncludedServiceItem(item)).length;
    const includedCount = items.length - billableCount;
    const fixedRows = items.map((item, index) => {
      const isIncluded = isOfferPdfIncludedServiceItem(item);
      return `
        <tr class="${isIncluded ? "is-included" : "is-billable"}">
          <td>${index + 1}</td>
          <td>
            ${formatOfferHtmlText(item.description || item.serviceCode || "Stavka")}
            ${isIncluded ? `<span class="offer-html-included-badge">Uključeno u plan</span>` : ""}
          </td>
          <td>${isIncluded ? "" : formatOfferHtmlText(item.unit || "")}</td>
          <td>${isIncluded ? "" : formatOfferHtmlText(item.quantity ?? "")}</td>
          <td>${isIncluded ? "" : escapeOfferHtml(formatOfferPdfCurrency(item.unitPrice ?? 0, currency))}</td>
          <td>${isIncluded ? `<span class="offer-html-included-badge">Uključeno</span>` : escapeOfferHtml(formatOfferPdfItemAmount(item, currency) || formatOfferPdfCurrency(item.totalPrice ?? 0, currency))}</td>
        </tr>
      `;
    }).join("");
    ...
    return `
      <div class="offer-html-fixed-plan-head">
        <strong>Fixed plan</strong>
        <span>${billableCount} naplativo · ${includedCount} uključeno</span>
      </div>
      ${buildOfferHtmlPlanFinancialSummary(offer, currency)}
      <table class="offer-html-items-table offer-html-fixed-plan-table">
        <thead>
  ...
  return `
    ${planType ? buildOfferHtmlPlanFinancialSummary(offer, currency) : ""}
    <table class="offer-html-items-table">
      ...
      ${showTotalAmount ? `<tfoot>${buildOfferHtmlFinancialFooterRows(offer, currency, { colspan: 5 })}</tfoot>` : ""}
    </table>
  `;
}
...
export async function buildOfferPdfBuffer(offer = {}, options = {}) {
  ...
  const drawOfferPdfTableCell = ({
    ...
    doc.restore();
    doc.save();
    doc.rect(x, y, width, height).lineWidth(0.55).strokeColor(borderColor).stroke();
    doc.restore();
    doc.font(bold ? "dejavu-bold" : "dejavu").fontSize(fontSize).fillColor(textColor).text(
  ...
  const drawOfferPdfSimplePriceTable = (rows = [], {
    emptyMessage = "Nema stavki za prikaz.",
  } = {}) => {
    const tableX = doc.page.margins.left;
    const tableWidth = helpers.availableWidth;
    const amountWidth = 132;
    const labelWidth = tableWidth - amountWidth;
    const drawHeader = () => {
      helpers.ensureSpace(31);
      const y = doc.y;
      drawOfferPdfTableCell({ x: tableX, y, width: labelWidth, height: 28, text: "Opis", bold: true, fillColor: "#f1f6ff", textColor: "#1e3a8a" });
      drawOfferPdfTableCell({ x: tableX + labelWidth, y, width: amountWidth, height: 28, text: "Iznos", align: "right", bold: true, fillColor: "#f1f6ff", textColor: "#1e3a8a" });
      doc.y = y + 28;
    };
    const safeRows = Array.isArray(rows) && rows.length > 0
      ? rows
      : [{ label: emptyMessage, amount: "", isMuted: true }];

    drawHeader();
    safeRows.forEach((row, index) => {
      const label = normalizePdfText(row?.label || "");
      const amount = normalizePdfText(row?.amount || "");
      const labelIndent = row?.indent ? 12 : 0;
      doc.font(row?.isGroup ? "dejavu-bold" : "dejavu").fontSize(row?.isGroup ? 9 : 8.6);
      const labelHeight = doc.heightOfString(label, { width: labelWidth - 12 - labelIndent, lineGap: 1 });
      const rowHeight = Math.max(28, labelHeight + 15);
      helpers.ensureSpace(rowHeight + 2);
      const y = doc.y;
      const fillColor = row?.isGroup ? "#f8fafc" : (index % 2 === 0 ? "#ffffff" : "#fbfdff");
      drawOfferPdfTableCell({
        ...
        height: rowHeight,
        text: label,
        bold: Boolean(row?.isGroup),
        fillColor,
        textColor: row?.isMuted ? "#64748b" : "#0f172a",
        paddingX: 6 + labelIndent,
      });
      drawOfferPdfTableCell({
        x: tableX + labelWidth,
        y,
        ...
        text: amount,
        align: "right",
        bold: Boolean(row?.isGroup || row?.isStrong),
        fillColor,
        textColor: row?.isMuted ? "#64748b" : "#0f172a",
      });
      doc.y = y + rowHeight;
  ...
  const drawOfferPdfFinancialItemsTable = (tableItems = [], {
    ...
    const tableWidth = helpers.availableWidth;
    const columns = [
      { id: "index", label: "#", width: 28, align: "center" },
      { id: "description", label: "Opis", width: Math.max(170, tableWidth - 28 - 44 - 44 - 105 - 112), align: "left" },
      { id: "unit", label: "Jed.", width: 44, align: "center" },
      { id: "quantity", label: "Kol.", width: 44, align: "center" },
      { id: "unitPrice", label: "Cijena", width: 105, align: "right" },
      { id: "amount", label: "Iznos", width: 112, align: "right" },
    ];
    const drawHeader = () => {
      helpers.ensureSpace(32);
      const y = doc.y;
      let x = tableX;
    ...
    const drawFooterRow = (row) => {
      const amountColumn = columns.at(-1);
      const labelWidth = tableWidth - amountColumn.width;
      const rowHeight = row.isGrand ? 34 : 29;
      helpers.ensureSpace(rowHeight + 2);
      const y = doc.y;
      const fillColor = row.isGrand ? "#ecfdf5" : "#f8fafc";
      const textColor = row.isGrand ? "#047857" : "#0f172a";
      drawOfferPdfTableCell({
        x: tableX,
        y,
        width: labelWidth,
    ...
    tableItems.forEach((item, index) => {
      ...
      const rowValues = {
        ...
        unit: isIncluded ? "" : clean(item.unit || ""),
        quantity: isIncluded ? "" : clean(item.quantity ?? ""),
        unitPrice: isIncluded ? "" : formatOfferPdfCurrency(item.unitPrice ?? 0, currency),
        amount: isIncluded
          ? "Uključeno"
          : (formatOfferPdfItemAmount(item, currency) || formatOfferPdfCurrency(item.totalPrice ?? 0, currency)),
      };
      doc.font("dejavu").fontSize(8.4);
      const descriptionColumn = columns.find((column) => column.id === "description");
      const descriptionHeight = doc.heightOfString(rowValues.description, {
      ...
      const y = doc.y;
      let x = tableX;
      const fillColor = isIncluded ? "#f3fbf6" : (index % 2 === 0 ? "#ffffff" : "#fbfdff");
      columns.forEach((column) => {
        drawOfferPdfTableCell({
          x,
          y,
          width: column.width,
          height: rowHeight,
          text: rowValues[column.id],
          align: column.align,
          bold: column.id === "amount" || column.id === "description",
          fontSize: column.id === "description" ? 8.4 : (column.id === "unitPrice" || column.id === "amount" ? 7.7 : 8.2),
          fillColor,
          textColor: isIncluded && column.id === "amount" ? "#047857" : "#0f172a",
        });
        x += column.width;
      });
      doc.y = y + rowHeight;
  ...
  const drawOfferPdfIncludedServicesTable = (includedItems = []) => {
    const tableX = doc.page.margins.left;
    const tableWidth = helpers.availableWidth;
    const columns = [
      { id: "index", label: "#", width: 30, align: "center" },
      { id: "code", label: "Sifra", width: 86, align: "left" },
      { id: "description", label: "Usluga ukljucena u pausal", width: Math.max(220, tableWidth - 116), align: "left" },
    ];
    const drawHeader = () => {
      helpers.ensureSpace(30);
      const y = doc.y;
    ...
    if (!Array.isArray(includedItems) || includedItems.length === 0) {
      ...
      drawOfferPdfTableCell({
        ...
        width: tableWidth,
        height: 28,
        text: "Nema dodanih ukljucenih usluga.",
        fillColor: "#ffffff",
        textColor: "#64748b",
      });
      doc.y = y + 28;
    ...
    includedItems.forEach((item, index) => {
      const rowValues = {
        index: String(index + 1),
        code: clean(item.serviceCode || ""),
        description: normalizePdfText(item.description || item.serviceCode || "Usluga"),
      };
      doc.font("dejavu").fontSize(8.4);
      const descriptionColumn = columns.find((column) => column.id === "description");
      const descriptionHeight = doc.heightOfString(rowValues.description, {
      ...
      const rowHeight = Math.max(30, descriptionHeight + 15);
      helpers.ensureSpace(rowHeight + 2);
      const y = doc.y;
      let x = tableX;
      const fillColor = index % 2 === 0 ? "#ffffff" : "#fbfdff";
      columns.forEach((column) => {
        drawOfferPdfTableCell({
          x,
          y,
          ...
          text: rowValues[column.id],
          align: column.align,
          bold: column.id === "description",
          fontSize: column.id === "description" ? 8.4 : 8,
          fillColor,
          textColor: "#0f172a",
        });
        x += column.width;
  ...
  if (items.length === 0) {
    ...
    if (true) {
      ...
      drawOfferPdfSimplePriceTable(monthlyItems.map((item) => {
        ...
        return {
          label: normalizePdfLines([
            item.description || "Mjesečna naknada",
            quantityText ? `Količina: ${quantityText}` : "",
          ]).join(" · "),
          amount: formatOfferPdfItemAmount(item, currency),
          isStrong: true,
        };
      }));
    }
    ...
    if (true) {
      ...
      const pricingRows = serviceItems.flatMap((item) => {
        ...
        if (breakdowns.length === 0) {
          return [{
            label: item.description || "Usluga",
            amount: formatOfferPdfItemAmount(item, currency),
            isStrong: true,
          }];
        }
        return [
          { label: item.description || "Usluga", amount: "", isGroup: true },
          ...breakdowns.map((entry) => ({
            label: getOfferPdfBreakdownDocumentLabel(entry),
            amount: formatOfferPdfCurrency(parseOfferPdfNumber(entry.amount, 0), currency),
            indent: true,
          })),
        ];
      });
      drawOfferPdfSimplePriceTable(pricingRows);
    }
  } else if (planType === "Fixed Plan") {
    const billableItems = items.filter((item) => !isOfferPdfIncludedServiceItem(item));
    const includedItems = items.filter((item) => isOfferPdfIncludedServiceItem(item));
    drawOfferPdfSectionTitle(doc, "Fiksne stavke ponude");
    drawOfferPdfFinancialItemsTable(billableItems);
    doc.moveDown(0.2);
  ...
  const offerTotalsRenderedInItemsTable = items.length > 0 && planType !== "Hybrid Plan";
  if (offer.showTotalAmount !== false && !offerTotalsRenderedInItemsTable) {
    helpers.ensureSpace(150);
    doc.moveDown(0.25);
    drawOfferPdfSectionTitle(doc, "Ukupni iznosi");
    getOfferPdfFinancialRows(offer, currency).forEach((row) => {
      writeOfferPdfMetaRow(doc, row.label, row.amount, {
        labelWidth: 150,
        valueWidth: 240,
      });
    });
  ...
  if (offer.showTotalAmount === false && items.length > 0) {
    doc.moveDown(0.35);
    doc.font("dejavu-italic").fontSize(9).fillColor("#64748b").text(
      "Ukupni iznos je skriven na dokumentu; stavke se prikazuju kao informativni cjenik.",
      { width: helpers.availableWidth },
    );
  }
