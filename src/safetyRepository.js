  ...
  return {
    test: {
      ...
      questionItems: visibleQuestions.map((question) => ({
        ...
        imageDocument: question.imageDocument ? cloneJsonValue(question.imageDocument) : null,
    ...
    assignment: {
      ...
      assigneeType: String(assignment.assigneeType ?? "user"),
      userId: String(assignment.userId ?? ""),
      userLabel: String(assignment.userLabel ?? ""),
      email: String(assignment.email ?? ""),
      externalFullName: String(assignment.externalFullName ?? ""),
      externalEmail: String(assignment.externalEmail ?? ""),
      externalPhone: String(assignment.externalPhone ?? ""),
      externalCompany: String(assignment.externalCompany ?? ""),
      externalOib: String(assignment.externalOib ?? ""),
      workOrderId: String(assignment.workOrderId ?? ""),
      workOrderNumber: String(assignment.workOrderNumber ?? ""),
      serviceId: String(assignment.serviceId ?? ""),
      serviceName: String(assignment.serviceName ?? ""),
      assignedByLabel: String(assignment.assignedByLabel ?? ""),
      safetySpecialistLabel: String(assignment.safetySpecialistLabel ?? ""),
...
function normalizeWorkOrderDocumentInput(input = {}, defaultSourceType = "editor") {
  ...
  return {
    ...
    signedFieldsJson: normalizeWorkOrderDocumentSignatureFieldsJson(
      input.signedFieldsJson ?? input.signedFields ?? input.signed_fields_json,
    ),
...
function normalizeWorkOrderDocumentPatch(input = {}) {
  ...
  if (
    Object.prototype.hasOwnProperty.call(input, "signedFieldsJson")
    || Object.prototype.hasOwnProperty.call(input, "signedFields")
    || Object.prototype.hasOwnProperty.call(input, "signed_fields_json")
  ) {
    patch.signedFieldsJson = normalizeWorkOrderDocumentSignatureFieldsJson(
      input.signedFieldsJson ?? input.signedFields ?? input.signed_fields_json,
    );
  }
...
function mapStoredAttachmentDocument(document = {}) {
  ...
  return {
    ...
    sourceDocumentId: dbString(document.sourceDocumentId ?? document.parentDocumentId),
    previewStatus: dbString(document.previewStatus),
    previewError: dbString(document.previewError),
...
function normalizeLinkedServiceCatalogIds(serviceCatalog = [], linkedServiceCatalogIds = [], organizationId = "") {
  const validServiceIds = new Set(
    (serviceCatalog ?? [])
      .filter((item) => !organizationId || String(item.organizationId) === String(organizationId))
  ...
  return Array.from(new Set(
    (linkedServiceCatalogIds ?? [])
      .map((value) => dbString(value))
      .filter((value) => validServiceIds.has(String(value))),
  ));
}

function deriveLinkedServiceCatalogIdsFromTemplateIds(serviceCatalog = [], linkedTemplateIds = [], organizationId = "") {
  ...
  return (serviceCatalog ?? [])
    .filter((item) => (
      ...
      && (item.linkedTemplateIds ?? []).some((templateId) => templateIdSet.has(String(templateId)))
    ))
    .map((item) => String(item.id))
    .filter(Boolean);
}

function getLinkedServiceCatalogTitles(serviceCatalog = [], linkedServiceCatalogIds = [], organizationId = "") {
  ...
  return Array.from(new Set(
    (linkedServiceCatalogIds ?? [])
      .map((serviceId) => {
        ...
        return dbString(service?.name || service?.serviceCode);
      })
      .filter(Boolean),
  ));
}

function deriveLinkedTemplateSnapshotFromServiceCatalog(serviceCatalog = [], linkedServiceCatalogIds = [], {
  fallbackTemplateIds = [],
  fallbackTemplateTitles = [],
  organizationId = "",
} = {}) {
  const servicesById = new Map(
    (serviceCatalog ?? [])
  ...
  if (linkedTemplateIds.length === 0 && linkedTemplateTitles.length === 0) {
    return {
      ...
      linkedTemplateTitles: Array.from(new Set(
        (fallbackTemplateTitles ?? [])
          .map((value) => dbString(value))
          .filter(Boolean),
      )),
...
function mapWorkOrderDocumentRow(row) {
  ...
  return {
    ...
    signedFieldsJson: row.signed_fields_json ?? row.signedFieldsJson ?? "",
...
async function fetchSnapshotFromConnection(connection) {
  ...
  const companies = companyRows.map((row) => {
    ...
    return {
      ...
      isznrTraining: parseJsonObject(row.isznr_training_json),
      period: row.periodika ?? "",
      isActive: normalizeActiveValue(row.aktivno),
      representative: row.predstavnik_korisnika ?? "",
  ...
  const [legalFrameworkRows] = await connection.query(`
    SELECT id, organization_id, title, category, authority_name, reference_code, version_label,
           ...
           linked_service_catalog_ids_json,
           created_at, updated_at
  ...
  const [serviceCatalogRows] = await connection.query(`
    SELECT id, organization_id, name, service_code, status, service_type, is_training, validity_months, linked_template_ids_json, linked_learning_test_ids_json, linked_qualification_keys_json, training_certificate_template_json, note, created_at, updated_at
    FROM web_service_catalog
    ORDER BY
      CASE status
        WHEN 'active' THEN 0
  ...
  const serviceCatalog = serviceCatalogRows.map((row) => {
    ...
    const linkedLearningTestTitles = learningTestIds
      ...
      .filter(Boolean);
    const linkedQualificationKeys = parseJsonArray(row.linked_qualification_keys_json).map((value) => dbString(value)).filter(Boolean);
  ...
  const legalFrameworks = legalFrameworkRows.map((row) => {
    const organizationId = dbString(row.organization_id);
    const legacyTemplateIds = documentTemplates
      .filter((template) => (
        String(template.organizationId) === String(organizationId)
        && (template.selectedLegalFrameworkIds ?? []).some((entryId) => String(entryId) === String(row.id))
      ))
      .map((template) => String(template.id))
      .filter(Boolean);
    const explicitServiceIds = normalizeLinkedServiceCatalogIds(
      serviceCatalog,
      parseJsonArray(row.linked_service_catalog_ids_json),
      organizationId,
    );
    const linkedServiceCatalogIds = explicitServiceIds.length > 0
      ? explicitServiceIds
      : deriveLinkedServiceCatalogIdsFromTemplateIds(serviceCatalog, legacyTemplateIds, organizationId);
    const linkedServiceCatalogTitles = getLinkedServiceCatalogTitles(serviceCatalog, linkedServiceCatalogIds, organizationId);
    const templateSnapshot = deriveLinkedTemplateSnapshotFromServiceCatalog(serviceCatalog, linkedServiceCatalogIds, {
      fallbackTemplateIds: legacyTemplateIds,
      fallbackTemplateTitles: legacyTemplateIds
        .map((templateId) => documentTemplatesById.get(String(templateId))?.title ?? "")
        .filter(Boolean),
      organizationId,
    });
  ...
  const measurementEquipment = measurementEquipmentRows.map((row) => {
    const organizationId = dbString(row.organization_id);
    const legacyTemplateIds = parseJsonArray(row.linked_template_ids_json).map((value) => dbString(value)).filter(Boolean);
    const explicitServiceIds = normalizeLinkedServiceCatalogIds(
      serviceCatalog,
      parseJsonArray(row.linked_service_catalog_ids_json),
      organizationId,
  ...
  const [safetyAuthorizationRows] = await connection.query(`
    SELECT id, organization_id, title, authorization_scope, issued_on, valid_until, valid_forever, note,
           linked_template_ids_json, linked_service_catalog_ids_json, documents_json, created_at, updated_at
...
export class InMemorySafetyRepository {
  ...
  async getSnapshot() {
    return {
      companies: this.snapshot.companies.map((item) => ({
        ...item,
      ...
      serviceCatalog: this.snapshot.serviceCatalog.map((item) => ({
        ...
        linkedQualificationKeys: [...(item.linkedQualificationKeys ?? [])],
  ...
  async addWorkOrderDocuments(workOrderId, files, actor = null, options = {}) {
    ...
    const nextDocuments = normalizedFiles.map((file, index) => ({
      ...
      signedFieldsJson: file.signedFieldsJson,
  ...
  async upsertWorkOrderGeneratedPdfDocument(workOrderId, file, actor = null) {
    ...
    const nextDocument = {
      ...
      signedFieldsJson: normalized.signedFieldsJson,
      signatureReviewStatus: "",
      signatureReviewComment: "",
      signatureReviewedByUserId: "",
      signatureReviewedByName: "",
  ...
  async updateWorkOrderDocument(workOrderId, documentId, patch, actor = null) {
    ...
    const next = {
      ...
      signedFieldsJson: Object.prototype.hasOwnProperty.call(normalizedPatch, "signedFieldsJson")
        ? normalizedPatch.signedFieldsJson
        : (current.signedFieldsJson || ""),
  ...
  async deleteServiceCatalogItem(id) {
    ...
    this.snapshot.legalFrameworks = this.snapshot.legalFrameworks.map((item) => ({
      ...item,
      linkedServiceCatalogIds: (item.linkedServiceCatalogIds ?? []).filter((entryId) => String(entryId) !== String(id)),
      linkedServiceCatalogTitles: [...getLinkedServiceCatalogTitles(
        this.snapshot.serviceCatalog,
        (item.linkedServiceCatalogIds ?? []).filter((entryId) => String(entryId) !== String(id)),
        item.organizationId,
      )],
      updatedAt: (item.linkedServiceCatalogIds ?? []).some((entryId) => String(entryId) === String(id))
        ? nowValue
        : item.updatedAt,
    }));
...
export class MySqlSafetyRepository {
  ...
  async init() {
    ...
    await ensureColumnExists(this.pool, "web_work_order_documents", "signed_fields_json", "LONGTEXT NULL AFTER signature_fields_json");
    await ensureColumnExists(this.pool, "web_work_order_documents", "signature_review_status", "VARCHAR(64) NOT NULL DEFAULT '' AFTER signed_fields_json");
    ...
    await this.pool.query(`
      CREATE TABLE IF NOT EXISTS web_legal_frameworks (
        ...
        note TEXT NULL,
        documents_json LONGTEXT NULL,
        linked_service_catalog_ids_json LONGTEXT NULL,
        created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    await ensureColumnExists(this.pool, "web_measurement_equipment", "linked_service_catalog_ids_json", "LONGTEXT NULL AFTER linked_template_ids_json");
    ...
    await this.pool.query(`
      CREATE TABLE IF NOT EXISTS web_service_catalog (
        ...
        validity_months VARCHAR(16) NOT NULL DEFAULT '',
        linked_template_ids_json LONGTEXT NULL,
        linked_learning_test_ids_json LONGTEXT NULL,
        linked_qualification_keys_json LONGTEXT NULL,
    ...
    await ensureColumnExists(this.pool, "web_service_catalog", "validity_months", "VARCHAR(16) NOT NULL DEFAULT '' AFTER is_training");
    ...
    await ensureColumnExists(this.pool, "web_service_catalog", "linked_qualification_keys_json", "LONGTEXT NULL AFTER linked_learning_test_ids_json");
    await ensureColumnExists(this.pool, "web_service_catalog", "training_certificate_template_json", "LONGTEXT NULL AFTER linked_qualification_keys_json");
    ...
    await ensureColumnExists(this.pool, "firme", "isznr_training_json", "LONGTEXT NULL AFTER template_assignments_json");
    ...
    try {
      ...
      const [result] = await connection.query(
        `
          INSERT INTO firme
            (naziv_tvrtke, sjediste, oib, mbs, nkd_djelatnost, predstavnik_korisnika, periodika, naziv_ugovora, vrsta_ugovora,
             ...
             voditelji_korisnik_ids_json, voditelji_oznaka_json, template_assignments_json, isznr_training_json,
             logo_data_url, logo_storage_provider, logo_storage_bucket, logo_storage_key, logo_storage_url,
             datum_izmjene, izmjenu_unio)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)
        `,
        [
          ...
          JSON.stringify(company.isznrTraining ?? {}),
      ...
      await connection.query(
        `
          ...
          SET naziv_tvrtke = ?, sjediste = ?, oib = ?, mbs = ?, nkd_djelatnost = ?, predstavnik_korisnika = ?, periodika = ?,
              ...
              isznr_training_json = ?, logo_data_url = ?, logo_storage_provider = ?, logo_storage_bucket = ?,
        ...
        [
          ...
          JSON.stringify(next.isznrTraining ?? {}),
          preparedLogo.storedLogo.dataUrl || null,
          preparedLogo.storedLogo.storageProvider || null,
          preparedLogo.storedLogo.storageBucket || null,
          preparedLogo.storedLogo.storageKey || null,
      ...
      for (const file of Array.isArray(files) ? files : []) {
        ...
        const [result] = await connection.query(
          `
            INSERT INTO web_work_order_documents
              (work_order_id, actor_user_id, actor_label, source_type, file_name, file_extension,
               ...
               preferred_field, signature_fields_json, signed_fields_json, file_size, data_url,
          ...
          [
            ...
            storedDocument.signedFieldsJson,
        ...
        createdDocuments.push({
          ...
          signedFieldsJson: storedDocument.signedFieldsJson,
      ...
      const [existingRows] = await connection.query(
        `
          SELECT id, work_order_id, actor_user_id, actor_label, source_type, file_name,
                 ...
                 signature_field_role, signature_field_oib, preferred_field, signature_fields_json, signed_fields_json,
      ...
      await connection.query(
        `
          ...
          SET file_name = ?, file_extension = ?, file_description = ?, document_category = ?,
              signed_fields_json = ?, signature_review_status = ?, signature_review_comment = ?, signature_reviewed_by_user_id = ?,
              signature_reviewed_by_name = ?, signature_reviewed_at = ?, signature_review_notified_owner = ?,
              updated_at = CURRENT_TIMESTAMP
          WHERE work_order_id = ? AND id = ?
        `,
      ...
      const [result] = await connection.query(
        `
          INSERT INTO web_service_catalog
            (organization_id, name, service_code, status, service_type, is_training, validity_months, linked_template_ids_json, linked_learning_test_ids_json, linked_qualification_keys_json, training_certificate_template_json, note)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `,
        [
          Number(draft.organizationId),
  ...
  async updateServiceCatalogItem(id, patch) {
    ...
    try {
      ...
      await connection.query(
        `
          UPDATE web_service_catalog
          SET organization_id = ?, name = ?, service_code = ?, status = ?, service_type = ?, is_training = ?, validity_months = ?, linked_template_ids_json = ?, linked_learning_test_ids_json = ?, linked_qualification_keys_json = ?, training_certificate_template_json = ?, note = ?
          WHERE id = ?
        `,
        [
          ...
          JSON.stringify(next.linkedQualificationKeys ?? []),
  ...
  async deleteServiceCatalogItem(id, { organizationId = "" } = {}) {
    ...
    try {
      ...
      for (const tableName of ["web_legal_frameworks", "web_measurement_equipment", "web_safety_authorizations"]) {
        const [rows] = await connection.query(
          `SELECT id, linked_service_catalog_ids_json FROM ${tableName} WHERE linked_service_catalog_ids_json IS NOT NULL`,
        );

        for (const row of rows) {
          const currentIds = parseJsonArray(row.linked_service_catalog_ids_json).map((entry) => dbString(entry)).filter(Boolean);
          if (!currentIds.some((entryId) => String(entryId) === String(id))) {
            continue;
          }
          ...
          await connection.query(
            `
              UPDATE ${tableName}
              SET linked_service_catalog_ids_json = ?, updated_at = CURRENT_TIMESTAMP
              WHERE id = ?
            `,
            [JSON.stringify(Array.from(new Set(nextIds))), Number(row.id)],
          );
    ...
    try {
      ...
      const preparedDocuments = await prepareStoredAttachmentDocuments(draft.documents, {
        ...
        [
          ...
          draft.note,
          JSON.stringify(draft.linkedTemplateIds ?? []),
          JSON.stringify(draft.linkedServiceCatalogIds ?? []),
          JSON.stringify(preparedDocuments.nextDocuments ?? []),
          JSON.stringify(draft.activityItems ?? []),
    ...
    try {
      ...
      const preparedDocuments = await prepareStoredAttachmentDocuments(next.documents, {
        ...
        [
          ...
          next.note,
          JSON.stringify(next.linkedTemplateIds ?? []),
          JSON.stringify(next.linkedServiceCatalogIds ?? []),
          JSON.stringify(preparedDocuments.nextDocuments ?? []),
          JSON.stringify(next.activityItems ?? []),
        ...
        `
          INSERT INTO web_safety_authorizations
            (organization_id, title, authorization_scope, issued_on, valid_until, valid_forever, note,
             linked_template_ids_json, linked_service_catalog_ids_json, documents_json)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `,
        [
          Number(draft.organizationId),
      ...
      await connection.query(
        `
          ...
          SET title = ?, authorization_scope = ?, issued_on = ?, valid_until = ?, valid_forever = ?,
              note = ?, linked_template_ids_json = ?, linked_service_catalog_ids_json = ?, documents_json = ?
          WHERE id = ?
        `,
        [
          next.title,
        ...
        `
          INSERT INTO web_legal_frameworks
            (organization_id, title, category, authority_name, reference_code, version_label,
             published_on, effective_from, review_date, status, tags_text, source_url, note, documents_json, linked_service_catalog_ids_json)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `,
        [
          Number(draft.organizationId),
      ...
      await connection.query(
        `
          ...
          SET title = ?, category = ?, authority_name = ?, reference_code = ?, version_label = ?,
              ...
              source_url = ?, note = ?, documents_json = ?, linked_service_catalog_ids_json = ?
          WHERE id = ?
        `,
        [
          next.title,
