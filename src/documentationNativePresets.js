...
function makeAiConfig({
  ...
  return {
    ...
    fallbackValue,
    confidenceRequired,
    sourcePriority: Array.isArray(sourcePriority) ? [...sourcePriority] : sourcePriority,
    sourceTracking: true,
    validationRules,
    displayOrder,
    group,
...
const EIZ_ZUDS_COLUMNS = [
  ...
  makeColumn("rcdRating", "In / Idn [A/mA]", 118),
  makeColumn("iisk", "Iisk [mA]", 84),
  makeColumn("tisk", "tisk [ms]", 84),
  makeColumn("u0", "U0 [V]", 76),
  makeColumn("pass", "Iisk < Idn / tisk < tdoz", 150, "DA/NE"),
];
...
const EIZ_MEASUREMENT_SOURCE_PRIORITY = Object.freeze([
  "previous_report",
...
const EIZ_MEASUREMENT_AI_AVOID = [
  "Ne koristi SPR, SZOM, TZIN, SVZ ili druge zapisnike za EIZ mjerne vrijednosti.",
  "Ne izmisljaj mjerne vrijednosti.",
  "Ako vrijednost nije jasno vidljiva u EIZ tablici, jednopolnoj shemi, parseru ili diktatu, ostavi prazno ili postojecu formulu.",
  "Ne mijesaj retke razlicitih razdjelnika, prostorija ili strujnih krugova.",
].join(" ");
const EIZ_MEASUREMENT_FORMULA_RULE = [
  "Ako korisnik diktira jednu baznu vrijednost za vise jednakih mjernih mjesta, smije se predloziti formula s RANDBETWEEN oko zadane vrijednosti.",
  "Za EIZ mjerenja koristi zadani raspon korisnika ili zadanu toleranciju plus/minus 5 posto.",
  "Ako korisnik nabroji vise vrijednosti, rasporedi ih redom po retcima i ne preskaci redove.",
].join(" ");
...
const EXEI_RELEVANT_CIRCUIT_TERMS = [
  "EXEI",
  "ExEi",
  "Ex prostor",
  "Ex zona",
...
const DOCUMENTATION_RESULT_AI_CONTEXT_BY_SERVICE = Object.freeze({
  ...
  EXEI: Object.freeze({
    subject: "elektricne instalacije u Ex prostoru",
    resultLookFor: ["EXEI", "ExEi", "Ex prostor", "IPK", "OI", "ZUDS", "PE", "Ex motor", "bimetal", ...EXEI_RELEVANT_CIRCUIT_TERMS, "zadovoljava"],
    defectsLookFor: ["nedostaci", "ne zadovoljava", "Ex", "motor", "bimetal", "preopterecenje", "impedancija", "otpor izolacije", "kontinuitet", ...EXEI_RELEVANT_CIRCUIT_TERMS],
    recommendationsLookFor: ["preporuke", "napomena", "sanacija Ex instalacije", "provjera Ex opreme", "provjera motora", "otkloniti nedostatke"],
    aiAvoid: "Ne koristi obicni EIZ, SPR, SZOM ili TZIN tekst za EXEI. Za EXEI koristi samo ExEi/EXEI zapisnik, Ex shemu ili ormare koji su vezani uz Ex prostor i Ex opremu.",
  }),
  VES: Object.freeze({
...
const DOCUMENTATION_COLUMN_AI_BY_TABLE = Object.freeze({
  ...
  "eiz-zuds": Object.freeze({
    board: makeColumnAi({
      key: "board",
      label: "Razdjelnik",
      aiAvoid: EIZ_MEASUREMENT_AI_AVOID,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
    ...
    rcdRating: makeColumnAi({
      key: "rcdRating",
      label: "In / Idn [A/mA]",
      type: "text",
      unit: "A/mA",
      aiAvoid: `${EIZ_MEASUREMENT_AI_AVOID} Ne dupliciraj In i Idn u odvojene kolone; za novi ZUDS predlozak vrati jednu vrijednost oblika 40/30.`,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
      aiDescription: "Pronadji FID/FI/ZUDS/RCD/RCCB/RCBO/KZS sklopku u EIZ.ZUDS tablici, jednopolnoj shemi ili slici elektroormara. Upisi jednu vrijednost u obliku In/Idn, npr. 40/30 ili 63/300.",
      aiLookFor: ["FID", "FI", "RCD", "RCCB", "RCBO", "KZS", "ZUDS", "40/30", "40A/30mA", "Idn", "I delta n", "mA", "In", ...EIZ_RCD_DEVICE_ALIASES, ...EIZ_RCD_RATING_ALIASES],
      examples: ["40/30", "63/300", "25/300"],
      validationRules: "Prvi broj prije / je nazivna struja u A, broj nakon / je diferencijalna struja u mA. Iz zapisa 40A/30mA vrati 40/30. Ako nedostaje jedan dio, ne izmisljaj ga i ne prepisuj istu vrijednost u drugi redak.",
    ...
    iisk: makeColumnAi({
      ...
      examples: ["23", "78", "210", "=IFERROR(IF(D3=\"\",\"\",RANDBETWEEN(70,80)*TEXTAFTER(D3,\"/\")/100),\"\")"],
      validationRules: `Ako je vrijednost prepisana iz starog zapisnika, sacuvaj decimalni separator i jedinicu ukloni. U novom ZUDS predlosku diferencijalna struja je dio nakon / u koloni In / Idn. ${EIZ_MEASUREMENT_FORMULA_RULE}`,
      confidenceRequired: "high",
    }),
    tisk: makeColumnAi({
      key: "tisk",
      ...
      examples: ["18", "24", "156", "=IFERROR(IF(D3=\"\",\"\",RANDBETWEEN(14,24)),\"\")"],
      validationRules: `Vrijednost mora biti u ms. ${EIZ_MEASUREMENT_FORMULA_RULE}`,
      confidenceRequired: "high",
    }),
    u0: makeColumnAi({
      key: "u0",
    ...
    pass: makeColumnAi({
      key: "pass",
      label: "Iisk < Idn / tisk < tdoz",
      type: "enum",
      aiAvoid: `${EIZ_MEASUREMENT_AI_AVOID} Ne upisuj DA samo zato sto je redak popunjen; DA smije biti prepisan ili jasno izracunat iz mjerenja.`,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
      ...
      commonValues: ["DA"],
      fallbackValue: "",
      validationRules: "Idn se cita iz dijela nakon / u koloni In / Idn. Ako je Iisk veci ili jednak Idn ili je tisk izvan dopustene granice, vrati NE. Ako nema dovoljno podataka, ostavi prazno ili postojecu formulu.",
      confidenceRequired: "high",
    }),
  }),
  "eiz-ipk": Object.freeze({
    ...
    zLpe: makeColumnAi({
      ...
      examples: ["1,42", "0.88", "=IF(B3=\"\",\"\",RANDBETWEEN(140,165)/100)"],
      validationRules: `Vrijednost mora ostati u ohmima. Ako korisnik diktira jednu ili vise baznih vrijednosti, rasporedi ih po retcima i koristi formule kada trazi raspon. ${EIZ_MEASUREMENT_FORMULA_RULE}`,
      confidenceRequired: "high",
    }),
    izem: makeColumnAi({
      key: "izem",
    ...
    zLn: makeColumnAi({
      ...
      examples: ["0,55", "0.63", "=IF(B3=\"\",\"\",RANDBETWEEN(55,63)/100)"],
      validationRules: `Vrijednost mora ostati u ohmima. Ako nije navedena, ne kopiraj Z(L-PE) osim ako korisnik izricito trazi kopiranje prethodnih vrijednosti. ${EIZ_MEASUREMENT_FORMULA_RULE}`,
      confidenceRequired: "high",
    }),
    zLl: makeColumnAi({
      key: "zLl",
      ...
      examples: ["-", "0,44", "=IF(B3=\"\",\"\",RANDBETWEEN(44,48)/100)"],
      validationRules: "Za jednofazna mjerna mjesta smije ostati crtica. Za 400 V/tri faze popuni samo ako je vrijednost vidljiva ili diktirana.",
      fallbackValue: "-",
    }),
    u0: makeColumnAi({
      key: "u0",
    ...
    pass: makeColumnAi({
      ...
      label: "ZADOVOLJAVA",
      type: "enum",
      aiAvoid: `${EIZ_MEASUREMENT_AI_AVOID} Ne upisuj DA samo zato sto postoji vrijednost; DA smije biti prepisan, formulom iz predloska ili jasno izracunat.`,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
      ...
      commonValues: ["DA"],
      fallbackValue: "",
      validationRules: "Ako stari zapisnik ima NE, zadrzi NE. Ako nema ocjene ili dovoljnih mjernih vrijednosti, ostavi prazno ili postojecu formulu.",
      confidenceRequired: "high",
    }),
  }),
  "eiz-oi": Object.freeze({
    circuit: makeColumnAi({
      ...
      aiAvoid: EIZ_MEASUREMENT_AI_AVOID,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
      aiDescription: "Prepiši oznaku strujnog kruga iz EIZ.OI tablice, slike elektroormara ili jednopolne sheme. Ako postoji samo slika ormara, upisi samo vidljivu oznaku osiguraca ili kruga.",
      aiLookFor: ["EIZ.OI", "otpor izolacije", "strujni krug", "oznaka kruga", "jednopolna shema", "F1", "QF", "1", "2", "osigurac"],
      examples: ["F1", "QF1", "1", "RO BS"],
      validationRules: "Ako je vidljiva samo slika ormara, ne izmisljaj naziv kruga: upisi samo oznaku osiguraca/strujnog kruga koja se jasno vidi, npr. F1, QF1 ili 1. Ako postoji shema, koristi oznaku kruga iz sheme.",
      required: true,
    }),
    conductor: makeColumnAi({
      ...
      aiAvoid: EIZ_MEASUREMENT_AI_AVOID,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
      aiDescription: "Prepiši vrstu vodica, oznaku vodica i vrstu/presjek kabela ako su navedeni u shemi ili OI tablici. Iz same fotografije ormara popuni samo ako je oznaka jasno citljiva.",
      aiLookFor: ["vrsta vodica", "oznaka vodica", "vrsta kabela", "presjek", "vodic", "kabel", "Cu", "Al", "PP-Y", "NYM", "NYY", "3x", "5x", "mm2"],
      examples: ["Cu", "PP-Y 3x2,5 mm2", "NYM-J 5x2,5 mm2 Cu"],
      validationRules: "Ako je dostupna shema, u kratkom zapisu navedi kabel/vodic i presjek. Ako podatak nije vidljiv, ostavi prazno; ne prepisuj naziv razdjelnika ili RCD podatke u ovu kolonu.",
    }),
    l123: makeColumnAi({
      key: "l123",
      ...
      aiAvoid: EIZ_MEASUREMENT_AI_AVOID,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
      aiDescription: "Prepiši izmjereni otpor izolacije izmedju faza L1-L2-L3 samo za tropolne/trofazne krugove.",
      aiLookFor: ["Riso L1-L2-L3", "L1-L2-L3", "MOhm", "Mohm", "3P", "tropolno", "trofazno", "400 V", "3x", "5x"],
      examples: [">30", "200", "50", "-"],
      validationRules: "Ako je krug jednopolni/jednofazni ili je vidljiv samo 230 V jednopolni osigurac, upisi -. Ako je tropolni/trofazni i Riso vrijednost je vidljiva, prepiši ju. Sacuvaj znak > i decimalni separator.",
      confidenceRequired: "high",
    }),
    l123n: makeColumnAi({
      key: "l123n",
      ...
      examples: [">30", "200"],
      validationRules: "Sacuvaj znak >, decimalni separator i redoslijed iz izvora. Ako je jednopolni/jednofazni krug i ova skupna vrijednost nije primjenjiva, upisi -. Ne popunjavaj iz drugih Riso kolona osim ako izvor jasno koristi istu skupnu vrijednost.",
      confidenceRequired: "high",
    }),
    l123pe: makeColumnAi({
      key: "l123pe",
      ...
      examples: [">30", "200"],
      validationRules: "Sacuvaj znak >, decimalni separator i redoslijed iz izvora. Ako je jednopolni/jednofazni krug i ova skupna vrijednost nije primjenjiva, upisi -. Ne popunjavaj ako je vidljiv samo IPK ili ZUDS redak.",
      confidenceRequired: "high",
    }),
    npe: makeColumnAi({
      key: "npe",
      ...
      examples: [">30", "200"],
      validationRules: "Sacuvaj znak >, decimalni separator i redoslijed iz izvora. Ako N-PE nije mjereno ili nije vidljivo, ostavi prazno/postojecu vrijednost.",
      confidenceRequired: "high",
    }),
    rd: makeColumnAi({
      key: "rd",
      ...
      examples: [">1", "1"],
      fallbackValue: ">1",
      validationRules: "Ako predlozak koristi Rd >1 Mohm i izvor nema drugu vrijednost, zadrzi >1. Ne pretvaraj u ohme.",
    }),
    pass: makeColumnAi({
      key: "pass",
      label: "Riso > Rd",
      type: "enum",
      aiAvoid: `${EIZ_MEASUREMENT_AI_AVOID} Ne upisuj DA ako Riso i Rd nisu jasno usporedivi.`,
      sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
      ...
      commonValues: ["DA"],
      fallbackValue: "",
      validationRules: "Ako je bilo koja obavezna Riso vrijednost manja ili jednaka Rd, vrati NE. Ako izvor nema mjerenja, ostavi prazno ili postojecu formulu.",
      confidenceRequired: "high",
    }),
  }),
  "eiz-k": Object.freeze({
  ...
  "exei-ipk": Object.freeze({
    circuit: makeExeiColumnAi({
      key: "circuit",
      label: "Oznaka strujnog kruga / el. uredjaja",
      aiDescription: "Pronadji Ex relevantni strujni krug ili uredjaj iz starog EXEI zapisnika, Ex sheme ili slike ormara. Prioritet su agregat, crpka, procesor agregata, rasvjeta agregata, UMP, motor i slicne Ex cjeline.",
      aiLookFor: ["oznaka strujnog kruga", "el. uredjaj", "ExEi.IPK", "ZOI-10-07", "Q", "F", "M", "agregat", "crpka", "UMP", "motor"],
      examples: ["Agregat", "Crpka", "Procesor agregata", "Rasvjeta agregata", "UMP", "Motor M1"],
      required: true,
    }),
    protectionDevice: makeExeiColumnAi({
      key: "protectionDevice",
      label: "Zastitni uredjaj",
      aiDescription: "Prepoznaj zastitni uredjaj za Ex krug: osigurac, motorna zastita, sklopka, RCD/ZUDS ili drugi uredjaj iz sheme/ormara.",
      aiLookFor: ["zastitni uredjaj", "osigurac", "motorna zastita", "sklopka", ...EIZ_RCD_DEVICE_ALIASES],
      examples: ["Automatski osigurac", "Motorna zastita", "FID 40/30"],
    }),
    protectionType: makeExeiColumnAi({
      key: "protectionType",
      label: "Tip i karakteristika",
      aiDescription: "Prepiši tip i karakteristiku zastitnog uredjaja za Ex IPK redak.",
      aiLookFor: ["tip i karakteristika", "B16", "C16", "D16", "motorna zastita", "40/30", ...EIZ_RCD_RATING_ALIASES],
      examples: ["C16", "B16", "Motorna zastita 1,6-2,5 A", "FID 40/30"],
    }),
    ia: makeExeiColumnAi({
      key: "ia",
      label: "Ia",
      type: "text",
      unit: "A",
      aiDescription: "Prepiši Ia iz starog EXEI/IPK zapisnika ili je ostavi formuli ako dolazi iz sifrarnika.",
      aiLookFor: ["Ia", "struja isklopa", "A", "ZOI-10-07"],
      validationRules: "Ne izracunavaj novu vrijednost iz sheme ako nije jasno zadana.",
    }),
    zLpe: makeExeiColumnAi({
      key: "zLpe",
      label: "Z(L-PE) [ohm]",
      type: "text",
      unit: "ohm",
      aiDescription: "Prepiši izmjerenu impedanciju petlje kvara Z(L-PE) samo iz starog EXEI/IPK zapisnika ili stvarnog mjerenja.",
      aiLookFor: ["Z(L-PE)", "impedancija", "petlja kvara", "ohm", "ZOI-10-07"],
      validationRules: "Shema ili slika ormara ne smiju popuniti stvarnu Z(L-PE) vrijednost.",
    }),
    zLn: makeExeiColumnAi({
      key: "zLn",
      label: "Z(L-N) [ohm]",
      type: "text",
      unit: "ohm",
      aiDescription: "Prepiši Z(L-N) samo iz starog EXEI/IPK zapisnika ili stvarnog mjerenja.",
      aiLookFor: ["Z(L-N)", "ohm", "ZOI-10-07"],
    }),
    zLl: makeExeiColumnAi({
      key: "zLl",
      label: "Z(L-L) [ohm]",
      type: "text",
      unit: "ohm",
      aiDescription: "Prepiši Z(L-L) za 3x/trofazne Ex krugove samo ako je stvarno mjerenje vidljivo.",
      aiLookFor: ["Z(L-L)", "3x", "trofazno", "400 V", "ohm"],
      examples: ["-", "0,42"],
      fallbackValue: "-",
    }),
    pass: makeExeiColumnAi({
      key: "pass",
      label: "Zadovoljava",
      type: "enum",
      aiDescription: "Prepiši DA/NE iz starog EXEI/IPK zapisnika. Ako postoji formula u predlosku, ne zamjenjuj ju bez jasnog izvora.",
      aiLookFor: ["zadovoljava", "DA", "NE", "ocjena"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
      fallbackValue: "",
    }),
  }),
  "exei-equipment": Object.freeze({
    device: makeExeiColumnAi({
      key: "device",
      label: "Mjerni uredjaj / tvornicki broj agregata",
      aiDescription: "Pronadji Ex motor, agregat, UMP, crpku ili drugu Ex opremu iz starog EXEI zapisnika, sheme ili slika. Ne uzimaj opcu opremu izvan Ex prostora.",
      aiLookFor: ["Ex motor", "agregat", "tvornicki broj agregata", "UMP", "crpka", "motor", "kompresor"],
      examples: ["Agregat A1", "Motor M1", "UMP", "Crpka"],
      required: true,
    }),
    manufacturerType: makeExeiColumnAi({
      key: "manufacturerType",
      label: "Proizvodjac / tip",
      aiDescription: "Prepiši proizvodjaca i tip Ex opreme ili motora s plocice/sheme/starog EXEI zapisnika.",
      aiLookFor: ["proizvodjac", "tip", "type", "manufacturer", "motor", "agregat"],
    }),
    motorSerial: makeExeiColumnAi({
      key: "motorSerial",
      label: "Tvornicki broj motora",
      aiDescription: "Prepiši tvornicki/serijski broj motora samo ako je vidljiv ili u starom EXEI zapisniku.",
      aiLookFor: ["tvornicki broj", "serijski broj", "serial", "S/N", "motor"],
    }),
    protectionCertificate: makeExeiColumnAi({
      key: "protectionCertificate",
      label: "Vrsta zastite / certifikat",
      aiDescription: "Prepiši Ex vrstu zastite i certifikat, npr. Ex d, Ex e, Ex de, ATEX/IECEx oznaku, samo ako je vidljivo.",
      aiLookFor: ["Ex d", "Ex e", "Ex de", "ATEX", "IECEx", "certifikat", "vrsta zastite"],
    }),
    inCurrent: makeExeiColumnAi({
      key: "inCurrent",
      label: "In",
      type: "text",
      unit: "A",
      aiDescription: "Prepiši nazivnu struju motora/opreme iz starog EXEI zapisnika ili plocice.",
      aiLookFor: ["In", "nazivna struja", "A", "motor"],
    }),
    pass: makeExeiColumnAi({
      key: "pass",
      label: "Ocjena",
      type: "enum",
      aiDescription: "Prepiši ocjenu DA/NE za Ex motor/opremu iz starog EXEI zapisnika.",
      aiLookFor: ["ocjena", "zadovoljava", "DA", "NE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-zuds": Object.freeze({
    board: makeExeiColumnAi({
      key: "board",
      label: "Razdjelnik",
      aiDescription: "Prepiši Ex razdjelnik/ormar za ZUDS/FID/RCD provjeru iz starog EXEI zapisnika, sheme ili slike ormara.",
      aiLookFor: ["razdjelnik", "ormar", "GRO", "RO", "ExEi.ZUDS", "FID", "RCD", "ZUDS"],
      required: true,
    }),
    circuit: makeExeiColumnAi({
      key: "circuit",
      label: "Strujni krug",
      aiDescription: "Prepiši Ex strujni krug povezan s RCD/ZUDS uredjajem. Ne dupliraj isti uredjaj ako se ponavlja na slici i shemi.",
      aiLookFor: ["strujni krug", "oznaka kruga", "FID", "RCD", "ZUDS", "Q", "F"],
      required: true,
    }),
    rcdRating: makeExeiColumnAi({
      key: "rcdRating",
      label: "In / Idn [A/mA]",
      type: "text",
      unit: "A/mA",
      aiDescription: "Vrati jednu vrijednost RCD/ZUDS uredjaja kao In/Idn, npr. 40/30 ili 63/300.",
      aiLookFor: ["40/30", "40A/30mA", "63/300", "Idn", "I delta n", ...EIZ_RCD_DEVICE_ALIASES, ...EIZ_RCD_RATING_ALIASES],
      examples: ["40/30", "63/300"],
      validationRules: "Dio prije / je In u A, dio nakon / je diferencijalna struja u mA. Ne vracaj samo 30 ako je vidljivo 40/30.",
    }),
    inCurrent: makeExeiColumnAi({
      key: "inCurrent",
      label: "In [A]",
      type: "text",
      unit: "A",
      aiDescription: "Ako tablica ima odvojenu kolonu, iz 40/30 upisi 40.",
      aiLookFor: ["In", "40/30", "40A/30mA"],
    }),
    idn: makeExeiColumnAi({
      key: "idn",
      label: "I delta n [mA]",
      type: "text",
      unit: "mA",
      aiDescription: "Ako tablica ima odvojenu kolonu, iz 40/30 upisi 30.",
      aiLookFor: ["Idn", "I delta n", "30mA", "300mA", "40/30"],
    }),
    iisk: makeExeiColumnAi({
      key: "iisk",
      label: "Iisk [mA]",
      type: "text",
      unit: "mA",
      aiDescription: "Prepiši Iisk samo iz starog EXEI/ZUDS zapisnika ili stvarnog mjerenja; ne iz sheme.",
      aiLookFor: ["Iisk", "struja prorade", "mA"],
    }),
    tisk: makeExeiColumnAi({
      key: "tisk",
      label: "tisk [ms]",
      type: "text",
      unit: "ms",
      aiDescription: "Prepiši tisk samo iz starog EXEI/ZUDS zapisnika ili stvarnog mjerenja; ne iz sheme.",
      aiLookFor: ["tisk", "vrijeme prorade", "ms"],
    }),
    pass: makeExeiColumnAi({
      key: "pass",
      label: "Zadovoljava",
      type: "enum",
      aiDescription: "Prepiši DA/NE iz starog EXEI/ZUDS zapisnika ili ostavi postojecu formulu.",
      aiLookFor: ["zadovoljava", "DA", "NE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-oi": Object.freeze({
    circuit: makeExeiColumnAi({
      key: "circuit",
      label: "Oznaka strujnog kruga",
      aiDescription: "Prepiši oznaku Ex strujnog kruga iz starog EXEI/OI zapisnika, sheme ili ormara.",
      aiLookFor: ["ExEi.OI", "otpor izolacije", "oznaka strujnog kruga", "F", "Q"],
      required: true,
    }),
    conductor: makeExeiColumnAi({
      key: "conductor",
      label: "Vrsta vodica",
      aiDescription: "Prepiši vrstu vodica/kabela i presjek samo ako su vidljivi u shemi ili starom EXEI/OI zapisniku.",
      aiLookFor: ["vrsta vodica", "kabel", "presjek", "Cu", "Al", "3x", "5x", "mm2"],
    }),
    l123: makeExeiColumnAi({
      key: "l123",
      label: "Riso L1-L2-L3 [Mohm]",
      type: "text",
      unit: "Mohm",
      aiDescription: "Prepiši stvarnu vrijednost samo iz starog EXEI/OI zapisnika. Iz sheme smijes vratiti '-' kada je krug 1x/jednofazni.",
      aiLookFor: ["Riso L1-L2-L3", "3x", "trofazno", "MOhm"],
      examples: [">100", "-"],
    }),
    l123n: makeExeiColumnAi({
      key: "l123n",
      label: "Riso L1-L2-L3-N [Mohm]",
      type: "text",
      unit: "Mohm",
      aiDescription: "Prepiši stvarnu vrijednost samo iz starog EXEI/OI zapisnika ili ostavi postojecu formulu.",
      aiLookFor: ["Riso L1-L2-L3-N", "MOhm"],
    }),
    l123pe: makeExeiColumnAi({
      key: "l123pe",
      label: "Riso L1-L2-L3-PE [Mohm]",
      type: "text",
      unit: "Mohm",
      aiDescription: "Prepiši stvarnu vrijednost samo iz starog EXEI/OI zapisnika ili ostavi postojecu formulu.",
      aiLookFor: ["Riso L1-L2-L3-PE", "PE", "MOhm"],
    }),
    npe: makeExeiColumnAi({
      key: "npe",
      label: "Riso N-PE [Mohm]",
      type: "text",
      unit: "Mohm",
      aiDescription: "Prepiši N-PE samo ako je vidljivo u starom EXEI/OI zapisniku ili stvarnom mjerenju.",
      aiLookFor: ["Riso N-PE", "N-PE", "MOhm"],
    }),
    pass: makeExeiColumnAi({
      key: "pass",
      label: "Riso > Rd",
      type: "enum",
      aiDescription: "Prepiši DA/NE iz starog EXEI/OI zapisnika ili ostavi postojecu formulu.",
      aiLookFor: ["Riso > Rd", "zadovoljava", "DA", "NE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-pe": Object.freeze({
    testPoint1: makeExeiColumnAi({
      key: "testPoint1",
      label: "Ispitno mjesto 1",
      aiDescription: "Prepiši prvo ispitno mjesto PE kontinuiteta u Ex prostoru.",
      aiLookFor: ["ExEi.PE", "kontinuitet", "ispitno mjesto 1", "PE", "metalne mase"],
      required: true,
    }),
    testPoint2: makeExeiColumnAi({
      key: "testPoint2",
      label: "Ispitno mjesto 2",
      aiDescription: "Prepiši drugo ispitno mjesto PE kontinuiteta u Ex prostoru.",
      aiLookFor: ["ispitno mjesto 2", "uzemljenje", "motor", "agregat", "metalna masa"],
      required: true,
    }),
    testCurrent: makeExeiColumnAi({
      key: "testCurrent",
      label: "Iisp [A]",
      type: "text",
      unit: "A",
      aiDescription: "Prepiši ispitnu struju samo ako je navedena u starom EXEI zapisniku ili mjerenju.",
      aiLookFor: ["Iisp", "ispitna struja", "A"],
      fallbackValue: "0,2",
    }),
    measuredResistance: makeExeiColumnAi({
      key: "measuredResistance",
      label: "Rizm [ohm]",
      type: "text",
      unit: "ohm",
      aiDescription: "Prepiši Rizm samo iz starog EXEI zapisnika ili stvarnog mjerenja.",
      aiLookFor: ["Rizm", "izmjereni otpor", "ohm"],
    }),
    pass: makeExeiColumnAi({
      key: "pass",
      label: "Rizm ~ Rocek",
      type: "enum",
      aiDescription: "Prepiši DA/NE iz starog EXEI PE zapisnika ili ostavi postojecu formulu.",
      aiLookFor: ["zadovoljava", "DA", "NE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-bimetal": Object.freeze({
    circuit: makeExeiColumnAi({
      key: "circuit",
      label: "Broj strujnog kruga",
      aiDescription: "Pronadji broj/oznaku strujnog kruga za Ex e ili Ex d motorni bimetal.",
      aiLookFor: ["broj strujnog kruga", "bimetal", "Ex e", "Ex d", "motor", "M"],
      required: true,
    }),
    protectionType: makeExeiColumnAi({
      key: "protectionType",
      label: "Tip i radno podrucje zastitnog uredjaja",
      aiDescription: "Prepiši tip i radno podrucje zastitnog uredjaja/bimetala za Ex motor.",
      aiLookFor: ["bimetal", "motorna zastita", "radno podrucje", "overload", "Ex e", "Ex d"],
    }),
    inCurrent: makeExeiColumnAi({
      key: "inCurrent",
      label: "In",
      type: "text",
      unit: "A",
      aiDescription: "Prepiši In za motor/bimetal iz starog EXEI zapisnika ili plocice.",
      aiLookFor: ["In", "nazivna struja", "A"],
    }),
    ip: makeExeiColumnAi({
      key: "ip",
      label: "Ip",
      type: "text",
      unit: "A",
      aiDescription: "Prepiši podesenu struju Ip zastitnog uredjaja ako je navedena.",
      aiLookFor: ["Ip", "podesena struja", "bimetal", "A"],
    }),
    pass: makeExeiColumnAi({
      key: "pass",
      label: "Zadovoljava",
      type: "enum",
      aiDescription: "Prepiši DA/NE za Ex bimetal iz starog EXEI zapisnika.",
      aiLookFor: ["zadovoljava", "DA", "NE", "ocjena"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-cista-ipk": Object.freeze({
    c1: makeExeiColumnAi({
      key: "circuit",
      label: "Oznaka strujnog kruga / el. uredjaja",
      aiDescription: "Upisi samo Ex relevantne krugove/opremu iz starog EXEI zapisnika ili pomocne sheme: agregat, crpka, procesor agregata, rasvjeta agregata, UMP, motor i slicno.",
      aiLookFor: ["ExEi1.2", "ZOI-10-07", "oznaka strujnog kruga", "el. uredjaj"],
      required: true,
    }),
    c2: makeExeiColumnAi({
      key: "phaseCount",
      label: "1x/3x",
      aiDescription: "Prepoznaj je li Ex krug jednofazni/jednopolni 1x ili trofazni/tropolni 3x prema shemi, zastitnom uredjaju ili starom EXEI zapisniku.",
      aiLookFor: ["1x", "3x", "jednofazno", "trofazno", "1P", "3P", "230 V", "400 V"],
      allowedValues: ["1x", "3x"],
      fallbackValue: "1x",
    }),
    c3: makeExeiColumnAi({
      key: "protectionType",
      label: "Tip i karakteristika",
      aiDescription: "Prepiši tip i karakteristiku zastitnog uredjaja za taj Ex krug.",
      aiLookFor: ["tip i karakteristika", "B16", "C16", "motorna zastita", ...EIZ_RCD_RATING_ALIASES],
    }),
    c7: makeExeiColumnAi({
      key: "zLpe",
      label: "Z(L-PE) [ohm]",
      type: "text",
      unit: "ohm",
      aiDescription: "Prepiši Z(L-PE) samo iz starog EXEI/IPK zapisnika ili stvarnog mjerenja; ne iz sheme.",
      aiLookFor: ["Z(L-PE)", "impedancija", "ohm"],
    }),
    c15: makeExeiColumnAi({
      key: "pass",
      label: "Zadovoljava DA/NE",
      type: "enum",
      aiDescription: "Prepiši ocjenu DA/NE iz starog EXEI/IPK zapisnika ili ostavi postojecu formulu.",
      aiLookFor: ["zadovoljava", "DA", "NE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
      fallbackValue: "",
  ...
  "exei-cista-oi": Object.freeze({
    c2: makeExeiColumnAi({
      key: "circuit",
      label: "Oznaka strujnog kruga",
      aiDescription: "Prepiši oznaku Ex strujnog kruga iz starog EXEI/OI zapisnika, Ex sheme ili ormara. Ako je samo slika ormara, upisi samo jasno vidljivu oznaku.",
      aiLookFor: ["ExEi1.3", "ZOI-10-03", "otpor izolacije", "oznaka strujnog kruga", "F", "Q"],
      required: true,
    }),
    c3: makeExeiColumnAi({
      key: "risoL123",
      ...
      type: "text",
      unit: "Mohm",
      aiDescription: "Prepiši vrijednost samo iz starog EXEI/OI zapisnika. Iz sheme smijes vratiti '-' za 1x/jednofazni krug.",
      aiLookFor: ["Riso L1-L2-L3", "MOhm", "3x", "trofazno"],
      examples: [">100", "-"],
    }),
    c4: makeExeiColumnAi({
      key: "risoL123N",
      ...
      type: "text",
      unit: "Mohm",
      aiDescription: "Prepiši vrijednost samo iz starog EXEI/OI zapisnika. Kod 1x/jednofaznog kruga smije biti >100 ili vrijednost iz formule ako je tako u predlosku.",
      aiLookFor: ["Riso L1-L2-L3-N", "MOhm", "N"],
    }),
    c5: makeExeiColumnAi({
      key: "risoL123Pe",
      label: "Riso L1-L2-L3-PE [Mohm]",
      type: "text",
      unit: "Mohm",
      aiDescription: "Prepiši Riso prema PE samo iz starog EXEI/OI zapisnika ili stvarnog mjerenja.",
      aiLookFor: ["Riso L1-L2-L3-PE", "PE", "MOhm"],
    }),
    c8: makeExeiColumnAi({
      key: "pass",
  ...
  "exei-cista-zuds": Object.freeze({
    c3: makeExeiColumnAi({
      key: "board",
      label: "Razdjelnik",
      aiDescription: "Prepiši Ex razdjelnik/ormar iz starog EXEI.ZUDS zapisnika, sheme ili slike ormara.",
      aiLookFor: ["razdjelnik", "ormar", "GRO", "RO", "ExEi1.4", "ZOI-10-08"],
      required: true,
    }),
    c6: makeExeiColumnAi({
      key: "circuit",
      label: "Strujni krug",
      aiDescription: "Prepiši oznaku Ex kruga povezanog s FID/RCD/ZUDS uredjajem. Ne dupliraj isti RCD.",
      aiLookFor: ["strujni krug", "FID", "RCD", "ZUDS", "Q", "F", ...EXEI_RELEVANT_CIRCUIT_TERMS],
      required: true,
    }),
    c8: makeExeiColumnAi({
      key: "inCurrent",
      ...
      type: "text",
      unit: "A",
      aiDescription: "Iz vrijednosti 40/30 ili 40A/30mA upisi dio prije /, tj. In u amperima.",
      aiLookFor: ["In", "40/30", "40A/30mA", ...EIZ_RCD_DEVICE_ALIASES],
      examples: ["40", "63"],
    }),
    c10: makeExeiColumnAi({
      key: "idn",
      label: "I Delta n [mA]",
      type: "text",
      unit: "mA",
      aiDescription: "Iz vrijednosti 40/30 ili 40A/30mA upisi dio nakon /, tj. I Delta n u mA.",
      aiLookFor: ["Idn", "I delta n", "30mA", "300mA", "40/30", ...EIZ_RCD_DEVICE_ALIASES],
      examples: ["30", "300"],
    }),
    c11: makeExeiColumnAi({
      key: "iisk",
      label: "Iisk [mA]",
      type: "text",
      unit: "mA",
    ...
    c17: makeExeiColumnAi({
      key: "pass",
      label: "Iisk < I Delta n / tisk < tdoz DA/NE",
      type: "enum",
      aiDescription: "Prepiši DA/NE iz starog EXEI/ZUDS zapisnika ili ostavi postojecu formulu.",
      aiLookFor: ["zadovoljava", "DA", "NE", "Iisk"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-cista-pe-direct": Object.freeze({
    c6: makeExeiColumnAi({
      key: "testPoint1",
      label: "Ispitno mjesto 1",
      aiDescription: "Prepiši prvo ispitno mjesto za kontinuitet PE vodica/metalnih masa u Ex prostoru.",
      aiLookFor: ["ExEi1.6", "ZOI-10-05", "ispitno mjesto 1", "PE", "metalne mase", ...EXEI_RELEVANT_CIRCUIT_TERMS],
      required: true,
    }),
    c7: makeExeiColumnAi({
      key: "testPoint2",
      label: "Ispitno mjesto 2",
      aiDescription: "Prepiši drugo ispitno mjesto za kontinuitet PE vodica/metalnih masa u Ex prostoru.",
      aiLookFor: ["ispitno mjesto 2", "uzemljenje", "metalna masa", "motor", "agregat"],
      required: true,
    }),
    c11: makeExeiColumnAi({
      key: "measuredResistance",
      ...
      type: "text",
      unit: "ohm",
      aiDescription: "Prepiši izmjereni otpor samo iz starog EXEI/PE zapisnika ili stvarnog mjerenja.",
      aiLookFor: ["Rizm", "izmjereni otpor", "ohm"],
    }),
    c16: makeExeiColumnAi({
      key: "pass",
      label: "Rizm ~ Rocek DA/NE",
      type: "enum",
      aiDescription: "Prepiši DA/NE za kontinuitet iz starog EXEI zapisnika ili ostavi formulu.",
      aiLookFor: ["zadovoljava", "DA", "NE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-cista-motors": Object.freeze({
    c1: makeExeiColumnAi({
      key: "device",
      label: "Mjerni uredaj / tvornicki broj agregata",
      aiDescription: "Upisi Ex motor, agregat, UMP, crpku ili drugu Ex opremu iz starog zapisnika, sheme ili plocice.",
      aiLookFor: ["ExEi1.7", "ZOI-10-14", "agregat", "motor", "UMP", "crpka", "tvornicki broj"],
      required: true,
    }),
    c2: makeExeiColumnAi({
      key: "manufacturerType",
      label: "Proizvodac / tip",
      aiDescription: "Prepiši proizvodjaca i tip Ex motora/opreme.",
      aiLookFor: ["proizvodjac", "tip", "type", "motor", "agregat"],
    }),
    c3: makeExeiColumnAi({
      key: "motorSerial",
      label: "Tvornicki broj motora",
      aiDescription: "Prepiši serijski/tvornicki broj motora ako je vidljiv.",
      aiLookFor: ["tvornicki broj", "serijski broj", "serial", "S/N"],
    }),
    c4: makeExeiColumnAi({
      key: "protectionCertificate",
      label: "Vrsta zastite / certifikat",
      aiDescription: "Prepiši Ex vrstu zastite i certifikat, npr. Ex d, Ex e, ATEX ili IECEx.",
      aiLookFor: ["Ex d", "Ex e", "ATEX", "IECEx", "certifikat"],
    }),
    c15: makeExeiColumnAi({
      key: "pass",
      label: "Ocjena DA/NE",
      type: "enum",
      aiDescription: "Prepiši ocjenu DA/NE iz starog EXEI zapisnika.",
      aiLookFor: ["ocjena", "zadovoljava", "DA", "NE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-cista-overload-e": Object.freeze({
    c2: makeExeiColumnAi({
      key: "circuit",
      label: "Broj strujnog kruga",
      aiDescription: "Prepiši broj/oznaku Ex e motornog kruga.",
      aiLookFor: ["ExEi1.8", "Ex e", "bimetal", "broj strujnog kruga", "motor"],
      required: true,
    }),
    c5: makeExeiColumnAi({
      key: "protectionType",
      label: "Tip i radno podrucje zastitnog uredjaja",
      aiDescription: "Prepiši tip i radno podrucje Ex e zastitnog uredjaja/bimetala.",
      aiLookFor: ["Ex e", "bimetal", "motorna zastita", "radno podrucje"],
    }),
    c21: makeExeiColumnAi({
      key: "pass",
      label: "tisk < tdoz / tisk < tE DA/NE",
      type: "enum",
      aiDescription: "Prepiši DA/NE iz starog EXEI Ex e bimetal zapisnika ili ostavi formulu.",
      aiLookFor: ["zadovoljava", "DA", "NE", "tE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
  }),
  "exei-cista-overload-d": Object.freeze({
    c2: makeExeiColumnAi({
      key: "circuit",
      label: "Broj strujnog kruga",
      aiDescription: "Prepiši broj/oznaku Ex d motornog kruga.",
      aiLookFor: ["ExEi1.9", "Ex d", "bimetal", "broj strujnog kruga", "motor"],
      required: true,
    }),
    c5: makeExeiColumnAi({
      key: "protectionType",
      label: "Tip i radno podrucje zastitnog uredjaja",
      aiDescription: "Prepiši tip i radno podrucje Ex d zastitnog uredjaja/bimetala.",
      aiLookFor: ["Ex d", "bimetal", "motorna zastita", "radno podrucje"],
    }),
    c18: makeExeiColumnAi({
      key: "pass",
      label: "tisk < tdoz / tisk < tE DA/NE",
      type: "enum",
      aiDescription: "Prepiši DA/NE iz starog EXEI Ex d bimetal zapisnika ili ostavi formulu.",
      aiLookFor: ["zadovoljava", "DA", "NE", "tE"],
      allowedValues: YES_NO_VALUES,
      commonValues: ["DA"],
    }),
...
function makeExeiColumnAi(options = {}) {
  const optionLookFor = Array.isArray(options.aiLookFor) ? options.aiLookFor : [];
  return makeColumnAi({
    aiAvoid: EXEI_STRUCTURE_AI_AVOID,
    sourcePriority: EIZ_MEASUREMENT_SOURCE_PRIORITY,
    confidenceRequired: "high",
    ...options,
    aiLookFor: Array.from(new Set(["EXEI", "ExEi", ...EXEI_RELEVANT_CIRCUIT_TERMS, ...optionLookFor].filter(Boolean))),
  });
}
...
export const DOCUMENTATION_NATIVE_REPORT_PRESETS = Object.freeze({
    ...
    tables: [
      tableSpec({
        ...
        rows: formulaRows(EIZ_ZUDS_COLUMNS, 10, (rowNumber, index) => {
          const inCurrent = index === 0 ? "63" : (index < 4 ? "40" : "25");
          const idn = [300, 30, 30, 30, 300, 300, 300, 300, 30, 30][index] || 30;
          const idnFormula = `TEXTAFTER(D${rowNumber},"/")`;
          return {
            board: "RO BS",
            circuit: ["F100.4", "F100.5", "F100.6", "F100.8", "F140", "F141", "F133", "F142", "F340", "F340.1"][index] || "",
            rcdRating: `${inCurrent}/${idn}`,
            iisk: `=IFERROR(IF(D${rowNumber}="","",RANDBETWEEN(70,80)*${idnFormula}/100),"")`,
            tisk: `=IFERROR(IF(D${rowNumber}="","",RANDBETWEEN(14,24)),"")`,
            u0: `=IF(D${rowNumber}="","","<50")`,
            pass: `=IF(D${rowNumber}="","","DA")`,
          };
        }),
      }),
      tableSpec({
  ...
  EXEI: createNativeReportPreset({
    ...
    tables: [
      ...
      tableSpec({ id: "exei-zuds", label: "ZUDS Ex", summary: "IL - ExEi.ZUDS", columns: EIZ_ZUDS_COLUMNS, blankRowCount: 8, blankSeed: { rcdRating: "40/30", pass: "DA" }, pageOrientation: "landscape" }),
