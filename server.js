...
const MOBILE_ANDROID_APK_FILE_NAME = "SafeNexus-0.1.98.apk";
...
function normalizeSignatureExportSettings(value = null) {
...
function getSignatureExportAppearanceSettings(signatureSettings = {}) {
  ...
  return safeSettings.appearance && typeof safeSettings.appearance === "object"
    ...
    : safeSettings.signatureAppearance;
}

function getSignatureExportRolePositioningSettings(signatureSettings = {}) {
  const safeSettings = normalizeSignatureExportSettings(signatureSettings);
  return safeSettings.rolePositioning && typeof safeSettings.rolePositioning === "object"
    ? safeSettings.rolePositioning
    : safeSettings.signaturePositioning;
}

async function addGeneratedTemplateDigitalSignatureFields(pdfBuffer = Buffer.alloc(0), entry = {}, signatureSettings = {}) {
...
function isCommercialMonthlyFeeItem(item = {}) {
...
function buildCommercialItemAmountText(item = {}, currency = "EUR") {
  ...
  const breakdownTotal = Array.isArray(item?.breakdowns)
    ? item.breakdowns.reduce((sum, entry) => sum + Math.max(0, Number(entry?.amount || 0)), 0)
    : 0;
  if (breakdownTotal > 0) {
    return getCommercialMoneyText(breakdownTotal, currency);
  }
...
function getCommercialOfferMonthlyItems(items = [], offerType = "") {
  ...
  if (planType === "Hybrid Plan") {
    return safeItems.filter((item) => isCommercialMonthlyFeeItem(item));
  }
...
function buildCommercialItemsTablePlaceholder(items = [], currency = "EUR", {
  ...
  const pushFinancialItemRow = (item, index) => {
    ...
    breakdowns.forEach((entry, breakdownIndex) => {
      const rowId = `financial-item-${index + 1}-breakdown-${breakdownIndex + 1}`;
      rows.push({
        id: rowId,
        cells: [
          buildCommercialTableCell(`- ${getCommercialBreakdownDocumentLabel(entry)}`, {
            align: "left",
            fillColor: "#F8FAFC",
            color: "#334155",
          }),
          buildCommercialTableCell("", { fillColor: "#F8FAFC" }),
          buildCommercialTableCell("", { fillColor: "#F8FAFC" }),
          buildCommercialTableCell("", { fillColor: "#F8FAFC" }),
          buildCommercialTableCell("", { fillColor: "#F8FAFC" }),
          buildCommercialTableCell(getCommercialMoneyText(entry?.amount, currency), {
            align: "right",
            bold: true,
            fillColor: "#F8FAFC",
            color: "#0F172A",
  ...
  const buildHybridMonthlyFeesTableBlock = (monthlyItems = []) => {
    const monthlyColumns = [
      { id: "index", label: "#", width: 42 },
      { id: "description", label: "Mjesečna naknada", width: 300 },
      { id: "unit", label: "Jed.", width: 54 },
      { id: "quantity", label: "Kol.", width: 54 },
      { id: "unitPrice", label: "Cijena", width: 90 },
      { id: "amount", label: "Iznos", width: 98 },
    ];
    const monthlyRows = [{
      id: "monthly-header",
      header: true,
      cells: monthlyColumns.map((column) => buildCommercialTableCell(column.label, {
        bold: true,
        fillColor: "#DBEAFE",
        color: "#1D4ED8",
        align: ["index", "unit", "quantity"].includes(column.id) ? "center" : (column.id === "amount" || column.id === "unitPrice" ? "right" : "left"),
      })),
    }];
    const monthlyMerges = [];
    if (monthlyItems.length === 0) {
      monthlyRows.push({
        id: "monthly-empty",
        cells: monthlyColumns.map((column, index) => buildCommercialTableCell(index === 0 ? "Nema mjesečnih naknada." : "", {
          color: "#64748B",
        })),
      });
      monthlyMerges.push({ rowId: "monthly-empty", columnId: "index", colSpan: monthlyColumns.length });
    } else {
      monthlyItems.forEach((item, index) => {
        monthlyRows.push({
          id: `monthly-fee-${index + 1}`,
          cells: [
            buildCommercialTableCell(String(index + 1), { align: "center" }),
            buildCommercialTableCell(item?.description || "Mjesečna naknada", { align: "left", bold: true }),
            buildCommercialTableCell(String(item?.unit || ""), { align: "center" }),
            buildCommercialTableCell(String(item?.quantity ?? ""), { align: "center" }),
            buildCommercialTableCell(getCommercialMoneyText(item?.unitPrice, currency), { align: "right" }),
            buildCommercialTableCell(buildCommercialItemAmountText(item, currency), { align: "right", bold: true }),
          ],
        });
      });
    }
    return buildTableBlock(monthlyColumns, monthlyRows, monthlyMerges, ["monthly-header"]);
  };

  const buildHybridServicePricingTableBlock = (serviceItems = []) => {
    const serviceColumns = [
      { id: "index", label: "#", width: 42 },
      { id: "description", label: "Usluga / razrada", width: 382 },
      { id: "unit", label: "Jedinica", width: 90 },
      { id: "amount", label: "Cijena", width: 120 },
    ];
    const serviceRows = [{
      id: "service-header",
      header: true,
      cells: serviceColumns.map((column) => buildCommercialTableCell(column.label, {
        ...
        fillColor: "#E0E7FF",
        color: "#3730A3",
        align: column.id === "index" || column.id === "unit" ? "center" : (column.id === "amount" ? "right" : "left"),
      })),
    }];
    const serviceMerges = [];
    if (serviceItems.length === 0) {
      serviceRows.push({
        id: "service-empty",
        cells: serviceColumns.map((column, index) => buildCommercialTableCell(index === 0 ? "Nema usluga u cjeniku." : "", {
          color: "#64748B",
        })),
      });
      serviceMerges.push({ rowId: "service-empty", columnId: "index", colSpan: serviceColumns.length });
    } else {
      serviceItems.forEach((item, index) => {
        ...
        if (breakdowns.length === 0) {
          serviceRows.push({
            ...
            cells: [
              buildCommercialTableCell(String(index + 1), { align: "center" }),
              buildCommercialTableCell(item?.description || item?.serviceCode || "Usluga", { align: "left", bold: true }),
              buildCommercialTableCell(getCommercialQuantityText(item) || item?.unit || "po usluzi", { align: "center" }),
              buildCommercialTableCell(buildCommercialItemAmountText(item, currency), { align: "right", bold: true }),
            ],
          });
          return;
        ...
        const groupRowId = `service-group-${index + 1}`;
        serviceRows.push({
          id: groupRowId,
          cells: [
            buildCommercialTableCell(String(index + 1), { align: "center", fillColor: "#F8FAFC" }),
            buildCommercialTableCell(item?.description || item?.serviceCode || "Usluga", { align: "left", bold: true, fillColor: "#F8FAFC" }),
            buildCommercialTableCell("", { fillColor: "#F8FAFC" }),
            buildCommercialTableCell("", { fillColor: "#F8FAFC" }),
          ],
        });
        serviceMerges.push({ rowId: groupRowId, columnId: "description", colSpan: 3 });
        breakdowns.forEach((entry, breakdownIndex) => {
          serviceRows.push({
            id: `service-item-${index + 1}-breakdown-${breakdownIndex + 1}`,
            cells: [
              buildCommercialTableCell("", { align: "center" }),
              buildCommercialTableCell(getCommercialBreakdownDocumentLabel(entry), { align: "left", color: "#334155" }),
              buildCommercialTableCell(entry?.unitLabel || "", { align: "center", color: "#475569" }),
              buildCommercialTableCell(getCommercialMoneyText(entry?.amount, currency), { align: "right", bold: true }),
            ],
          });
        });
      });
    }
    return buildTableBlock(serviceColumns, serviceRows, serviceMerges, ["service-header"]);
  };

  const buildHybridTotalsTableBlock = () => {
    const totalColumns = [
      { id: "label", label: "Ukupni iznosi", width: 500 },
      { id: "amount", label: "Iznos", width: 130 },
    ];
    const totalRows = [{
      id: "totals-header",
      header: true,
      cells: totalColumns.map((column) => buildCommercialTableCell(column.label, {
        bold: true,
        fillColor: "#F3F4F6",
        align: column.id === "amount" ? "right" : "left",
      })),
    ...
    return buildTableBlock(totalColumns, totalRows, [], ["totals-header"]);
  };

  if (isHybridPlan) {
    const monthlyItems = getCommercialOfferMonthlyItems(allItems, offerType);
    const serviceItems = getCommercialOfferServicePricingItems(allItems, offerType);
    if (section === "monthly") {
      return buildHybridMonthlyFeesTableBlock(monthlyItems);
    }
    if (section === "services") {
      return buildHybridServicePricingTableBlock(serviceItems);
    }
    if (section === "all") {
      const blocks = [
        { type: "heading", text: "Mjesečne naknade", level: 4 },
        buildHybridMonthlyFeesTableBlock(monthlyItems),
        { type: "heading", text: "Cjenik usluga po izvršenju", level: 4 },
        buildHybridServicePricingTableBlock(serviceItems),
      ];
      if (showTotalAmount && totals && typeof totals === "object") {
        blocks.push(
          { type: "heading", text: "Ukupni iznosi", level: 4 },
          buildHybridTotalsTableBlock(),
        );
      }
      return { __docxBlockType: "blocks", blocks };
    }
  }
  ...
  if (isFinancialPlan) {
    ...
    if (feeItems.length > 0) {
      pushMergedRow("monthly-fees-section", "Mjesečne naknade", { fillColor: "#DBEAFE", color: "#1D4ED8", bold: true });
      feeItems.forEach((item, index) => {
        pushRow(
          `monthly-fee-${index + 1}`,
          item?.description || "Mjesečna naknada",
          buildCommercialItemAmountText(item, currency),
          { fillColor: "#EFF6FF", bold: true },
        );
      });
    }

    if (serviceItems.length > 0) {
      pushMergedRow("service-pricing-section", "Cjenik usluga", { fillColor: "#E0E7FF", color: "#3730A3", bold: true });
      serviceItems.forEach((item, index) => {
        pushMergedRow(`service-item-${index + 1}`, item?.description || "Usluga", { fillColor: "#E0F2FE", color: "#0F172A", bold: true });
        pushBreakdownRows(item, index);
      });
    }
...
async function saveGeneratedDocumentTemplatePdfDocuments(entries = [], scopedSnapshot = {}, user = null, options = {}) {
  const startedAt = Date.now();
...
function buildOfferTemplatePlaceholderPayload(offer = {}) {
  ...
  const itemsFallbackTitle = planType === "Per Employee Plan"
    ? "Cijena po zaposleniku"
    : "Opis stavke";
  ...
  const itemsTable = buildCommercialItemsTablePlaceholder(items, currency, {
    fallbackTitle: itemsFallbackTitle,
...
function resolveWorkOrderExecutorsPayload(scopedSnapshot = {}, input = {}) {
  const rawValues = Array.isArray(input.executors)
    ? input.executors
    : [input.executor1, input.executor2];
  const users = scopedSnapshot.users ?? [];
  const usersById = new Map(users.map((user) => [String(user.id), user]));
  const usersByLabel = new Map(users.flatMap((user) => {
    const label = getScopedUserDisplayLabel(user);
    return label ? [[label.toLowerCase(), user]] : [];
  }));
  ...
  return rawValues
    ...
    .filter(Boolean)
    .map((value) => {
      const idMatch = usersById.get(String(value));
      if (idMatch) {
        return getScopedUserDisplayLabel(idMatch, value);
      }
      const labelMatch = usersByLabel.get(value.toLowerCase());
      return labelMatch ? getScopedUserDisplayLabel(labelMatch, value) : value;
    })
    .filter(Boolean)
...
function resolveFieldInquiryExecutorsForWorkOrder(scopedSnapshot = {}, inquiry = {}) {
  const fromIds = resolveWorkOrderExecutorsPayload(scopedSnapshot, {
    executors: Array.isArray(inquiry.assignedUserIds) ? inquiry.assignedUserIds : [],
  });
  if (fromIds.length > 0) {
    return fromIds;
  }
  return resolveWorkOrderExecutorsPayload(scopedSnapshot, {
    executors: Array.isArray(inquiry.assignedUserLabels) ? inquiry.assignedUserLabels : [],
  });
}
...
function getScopedUserDisplayLabel(userLike = {}, fallback = "User") {
  return String(
    userLike.fullName
...
function normalizeMobileSystemDescriptionRuntimeValue(field = {}, value = null) {
  if (typeof value === "string") {
    ...
    if ((trimmedValue.startsWith("{") && trimmedValue.endsWith("}")) || (trimmedValue.startsWith("[") && trimmedValue.endsWith("]"))) {
      try {
        return normalizeMobileSystemDescriptionRuntimeValue(field, JSON.parse(trimmedValue));
      } catch {
        // Fall through and treat it as normal text.
      }
    }
  }
...
function normalizeMobileAiConfigList(value = [], maxItems = 120) {
  const source = Array.isArray(value)
    ? value
    : String(value || "")
      .split(/\r?\n|,/);
  return source
    .map((entry) => normalizeInputValue(entry).slice(0, 240))
    .filter(Boolean)
    .slice(0, maxItems);
}

function normalizeMobileAiFieldType(value = "", fallback = "text") {
  const normalized = normalizeInputValue(value || fallback).toLowerCase();
  const allowed = new Set(["text", "number", "date", "enum", "boolean", "list", "table", "system_description"]);
  return allowed.has(normalized) ? normalized : fallback;
}

function normalizeMobileAiConfidence(value = "", fallback = "medium") {
  const normalized = normalizeInputValue(value || fallback).toLowerCase();
  ...
  const fallbackType = fieldType === "dropdown"
    ...
    : fieldType === "measurement_table"
      ...
      : fieldType === "date"
        ? "date"
        : "text";
  const aiDescription = normalizeInputValue(source?.aiDescription ?? source?.ai_description ?? source?.description).slice(0, 2000);
  const aiLookFor = normalizeMobileAiConfigList(source?.aiLookFor ?? source?.ai_look_for, 160);
  const aiAvoid = normalizeInputValue(source?.aiAvoid ?? source?.ai_avoid).slice(0, 1000);
  return {
    key: normalizeInputValue(source?.key ?? field?.key).slice(0, 120),
    label: normalizeInputValue(source?.label ?? field?.label ?? field?.wordLabel).slice(0, 160),
    description: normalizeInputValue(source?.description ?? field?.helpText).slice(0, 2000),
    type: normalizeMobileAiFieldType(source?.type || fallbackType, fallbackType),
    required: Boolean(source?.required || field?.required || field?.isRequired),
    placeholder: normalizeInputValue(source?.placeholder).slice(0, 400),
    helpText: normalizeInputValue(source?.helpText ?? source?.help_text ?? field?.helpText).slice(0, 1000),
    enabled: Boolean(source?.enabled ?? source?.aiEnabled ?? source?.ai_enabled),
    aiDescription,
    aiLookFor,
    aiAvoid,
    allowedValues: normalizeMobileAiConfigList(source?.allowedValues ?? source?.allowed_values, 160),
    commonValues: normalizeMobileAiConfigList(source?.commonValues ?? source?.common_values, 80),
    examples: normalizeMobileAiConfigList(source?.examples, 80),
    format: normalizeInputValue(source?.format).slice(0, 1200),
    unit: normalizeInputValue(source?.unit).slice(0, 40),
    defaultValue: normalizeInputValue(source?.defaultValue ?? source?.default_value ?? field?.defaultValue).slice(0, 500),
    fallbackValue: normalizeInputValue(source?.fallbackValue ?? source?.fallback_value).slice(0, 500),
    confidenceRequired: normalizeMobileAiConfidence(source?.confidenceRequired ?? source?.confidence_required, "medium"),
    sourceTracking: Boolean(source?.sourceTracking ?? source?.source_tracking ?? true),
    validationRules: normalizeInputValue(source?.validationRules ?? source?.validation_rules).slice(0, 1600),
    displayOrder: Number.isFinite(Number(source?.displayOrder ?? source?.display_order))
...
function hasMobileDocumentTemplateFieldAiConfig(input = {}, field = {}) {
  const config = normalizeMobileDocumentTemplateFieldAiConfig(input, field);
  return Boolean(
    config.enabled
...
function normalizeMobileMeasurementColumnAiMapping(input = {}) {
  const source = input && typeof input === "object" && !Array.isArray(input) ? input : {};
  const format = normalizeMobileAiFieldType(source?.format || source?.type || "text", "text");
  const aiDescription = normalizeInputValue(source?.aiDescription ?? source?.ai_description ?? source?.description).slice(0, 4000);
  const aiLookFor = normalizeMobileAiConfigList(source?.aiLookFor ?? source?.ai_look_for ?? source?.synonyms, 160);
  const aiAvoid = normalizeInputValue(source?.aiAvoid ?? source?.ai_avoid ?? source?.avoid).slice(0, 1000);
  return {
    key: normalizeInputValue(source?.key).slice(0, 120),
    label: normalizeInputValue(source?.label).slice(0, 160),
    description: normalizeInputValue(source?.description ?? source?.aiDescription ?? source?.ai_description).slice(0, 2000),
    type: normalizeMobileAiFieldType(source?.type || format, format),
    required: Boolean(source?.required),
    placeholder: normalizeInputValue(source?.placeholder).slice(0, 400),
    helpText: normalizeInputValue(source?.helpText ?? source?.help_text).slice(0, 1000),
    enabled: Boolean(source?.enabled ?? source?.aiEnabled ?? source?.ai_enabled),
    aiDescription,
    aiLookFor,
    aiAvoid,
    synonyms: normalizeMobileAiConfigList(source?.synonyms ?? aiLookFor, 80),
    allowedValues: normalizeMobileAiConfigList(source?.allowedValues ?? source?.allowed_values, 160),
    commonValues: normalizeMobileAiConfigList(source?.commonValues ?? source?.common_values, 80),
    examples: normalizeMobileAiConfigList(source?.examples, 80),
    avoid: aiAvoid,
    format,
    unit: normalizeInputValue(source?.unit).slice(0, 40),
    defaultValue: normalizeInputValue(source?.defaultValue ?? source?.default_value).slice(0, 500),
    fallbackValue: normalizeInputValue(source?.fallbackValue ?? source?.fallback_value).slice(0, 500),
    confidenceRequired: normalizeMobileAiConfidence(source?.confidenceRequired ?? source?.confidence_required, "medium"),
    sourceTracking: Boolean(source?.sourceTracking ?? source?.source_tracking ?? true),
    validationRules: normalizeInputValue(source?.validationRules ?? source?.validation_rules).slice(0, 1600),
...
function hasMobileMeasurementColumnAiMapping(input = {}) {
  const mapping = normalizeMobileMeasurementColumnAiMapping(input);
  return Boolean(
    mapping.description
    || mapping.key
    || mapping.label
    ...
    || mapping.aiLookFor.length
    || mapping.allowedValues.length
    || mapping.commonValues.length
    || mapping.examples.length
    || mapping.avoid
    || mapping.aiAvoid
    || mapping.format !== "text"
    || mapping.type !== "text"
...
function buildMobileDocumentTemplateAiFields(template = {}) {
  return (Array.isArray(template?.customFields) ? template.customFields : [])
    .map((field, index) => {
      ...
      const key = normalizeInputValue(config.key || field?.key || tokenKey);
      const label = normalizeInputValue(config.label || field?.label || field?.wordLabel || key || `Polje ${index + 1}`);
      if (!id && !key) {
        return null;
      }
      const runtimeField = {
      ...
      if (isSystemDescription) {
        ...
        runtimeField.sectionSubtitle = normalizeInputValue(field?.sectionSubtitle || field?.section_subtitle);
        runtimeField.systemRows = getMobileSystemDescriptionTemplateRows(field).map((row, rowIndex) => ({
          id: normalizeInputValue(row?.id) || `system-description-row-${rowIndex + 1}`,
          subtitle: normalizeInputValue(row?.subtitle),
          lineCount: normalizeMobileSystemDescriptionLineCount(row?.lineCount),
          placeholder: normalizeInputValue(row?.placeholder),
        }));
      }
      return runtimeField;
    })
...
function buildMobileDocumentTemplateAiMeasurementColumns(template = {}, workOrder = {}, service = {}, scopedSnapshot = {}, common = {}, fieldSheets = {}) {
  ...
  (Array.isArray(template?.customFields) ? template.customFields : []).forEach((field, fieldIndex) => {
    ...
    const tokenKey = getMobileDocumentTemplateFieldTokenKey(field, fieldIndex);
    const recordKey = normalizeInputValue(field?.key || field?.id || tokenKey);
    const sheet = normalizeWorkOrderMeasurementSheet(fieldSheets?.[recordKey])
      || resolveMobileDocumentTemplateFieldSheet(template, workOrder, service, scopedSnapshot, common, field, fieldIndex);
    ...
    sheet.columns
      ...
      .forEach((column, columnIndex) => {
        ...
        columns.push({
          fieldId: normalizeInputValue(field?.id || recordKey || tokenKey),
          fieldKey: recordKey,
          fieldLabel: normalizeInputValue(field?.label || field?.wordLabel || recordKey || "Mjerenje"),
          fieldDescription: fieldAi.aiDescription || fieldAi.description || normalizeInputValue(field?.helpText),
          columnId: normalizeInputValue(column?.id || `measurement-column-${columnIndex + 1}`),
          columnIndex,
          columnLetter: getMobileSpreadsheetColumnLabel(columnIndex),
          key: aiMapping.key || normalizeInputValue(column?.id),
          label: aiMapping.label || normalizeInputValue(column?.label || column?.placeholder || column?.id || "Kolona"),
          type: aiMapping.type || aiMapping.format || "text",
          required: Boolean(aiMapping.required),
          placeholder: normalizeInputValue(column?.placeholder || aiMapping.placeholder),
...
async function handleApiRequest(request, response, url) {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/offers") {
      ...
      assertCompanyPayloadInScope(scopedSnapshot, body);
      assertLocationPayloadInScope(scopedSnapshot, body);
      const created = await domainRepository.createOffer({
        ...body,
        organizationId: scopedSnapshot.activeOrganizationId,
      }, user);
      if (!isSnapshotCacheableRequest(request)) {
