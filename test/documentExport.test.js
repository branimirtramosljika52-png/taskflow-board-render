...
test("docx export renders portrait risk assessment cards without landscape sections", async () => {
  const templateBuffer = buildMinimalDocxBuffer(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
    <w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
      <w:body>
        <w:p><w:r><w:t>{{RISK_JOBS}}</w:t></w:r></w:p>
    ...
    RISK_JOBS: {
      ...
      blocks: [
        { type: "heading", text: "Opasnosti, stetnosti, napori i mjere", level: 4 },
        {
          ...
          headerRows: ["h", "risk-1-labels", "risk-1-analysis-head"],
          merges: [
            { rowId: "h", columnId: "probability", colSpan: 4 },
            { rowId: "risk-1-title", columnId: "probability", colSpan: 4 },
            { rowId: "risk-1-analysis-head", columnId: "probability", colSpan: 2 },
            { rowId: "risk-1-analysis-head", columnId: "matrix", colSpan: 2 },
            { rowId: "risk-1-analysis", columnId: "probability", colSpan: 2 },
            { rowId: "risk-1-analysis", columnId: "matrix", colSpan: 2 },
          ],
          rows: [
            { id: "h", header: true, cells: [{ text: "Opasnosti, stetnosti, napori i mjere", format: { fontSize: 9, bold: true } }] },
            { id: "risk-1-title", cells: [{ text: "I. OPASNOSTI / 1. MEHANICKE OPASNOSTI\n1.2 Strojevi i oprema\nMoguce posljedice: ozljede", format: { fontSize: 8.1, bold: true } }] },
            {
              id: "risk-1-labels",
              header: true,
              cells: [
                { text: "Vjerojatnost", format: { fontSize: 8.2, bold: true } },
                { text: "Posljedice", format: { fontSize: 8.2, bold: true } },
                { text: "Rizik", format: { fontSize: 8.2, bold: true } },
                { text: "Posebni uvjeti", format: { fontSize: 8.2, bold: true } },
              ],
            },
            {
              id: "risk-1-values",
              cells: [
                { text: "Srednja vjerojatnost", format: { fontSize: 7.4, align: "center", bold: true, card: { fillColor: "#FFFFFF", borderColor: "#CBD5E1" } } },
                { text: "Velika posljedica", format: { fontSize: 7.4, align: "center", bold: true, card: { fillColor: "#FFFFFF", borderColor: "#CBD5E1" } } },
                { text: "Veliki rizik (3)", format: { fontSize: 7.4, align: "center", bold: true, card: { fillColor: "#FEE2E2", borderColor: "#94A3B8" } } },
                { text: "Ne", format: { fontSize: 7.4, align: "center" } },
              ],
            },
            { id: "risk-1-analysis-head", header: true, cells: [{ text: "Objasnjenje / napomena", format: { fontSize: 8.2, bold: true } }, {}, { text: "Primijenjena pravila, mjere i postupci", format: { fontSize: 8.2, bold: true } }, {}] },
            { id: "risk-1-analysis", cells: [{ text: "-", format: { fontSize: 7.6 } }, {}, { text: "Raditi samo na ispravnoj opremi.", format: { fontSize: 7.6 } }, {}] },
          ],
        },
      ],
    },
  ...
  assert.equal(outputXml.includes('w:orient="landscape"'), false);
  assert.match(outputXml, /Opasnosti, stetnosti, napori i mjere/);
  assert.match(outputXml, /Srednja vjerojatnost/);
  assert.match(outputXml, /Veliki rizik \(3\)/);
  assert.match(outputXml, /w:gridSpan w:val="4"/);
  assert.match(outputXml, /w:gridSpan w:val="2"/);
  assert.match(outputXml, /w:fill="FEE2E2"/);

  const fallbackHtml = (await convertWordBufferToHtmlTemplate(outputBuffer, {
    fileName: "procjena-rizika.docx",
    allowLibreOfficeFallback: false,
  })).html;
  assert.match(fallbackHtml, /colspan="4"/);
  assert.match(fallbackHtml, /colspan="2"/);
  assert.match(fallbackHtml, /background-color:#FEE2E2/);
  assert.doesNotMatch(fallbackHtml, /data-word-orientation="landscape"/);
});
