...
class SafeNexusApi(
    ...
    private fun workEquipmentRoTemplateAssessmentRule(batchMode: Boolean): String =
        listOf(
            ...
            "Primarni zadatak je samo: 1) prepoznati najblizi profileId/profileName iz context.profiles, 2) procitati natpisnu plocicu i osnovne oznake, 3) vratiti pitanja za rucnu potvrdu ako predlozak ili plocica nisu sigurni.",
            ...
            "NE popunjavaj opisna polja iz slike osim name/manufacturer/model/serialNumber/inventoryNumber/technicalData. Namjena, polozaj, radne tvari, uporaba/odrzavanje, metode, nedostaci, mjere i zavrsna ocjena dolaze iz Android predloska.",
            "NE vracaj strojarske, elektro, opasnosti, stetnosti ni napore u template nacinu. Vrati mechanicalItems=[], electricalItems=[], hazardRegisterIris=[], harmfulnessRegisterIris=[] i strainRegisterIris=[]. Android ce sve te stavke popuniti iz prepoznatog predloska.",
            "Ako bi neka stavka trebala drugaciji upis od predloska, ne upisuj je u mechanicalItems/electricalItems; samo dodaj jasno pitanje u verificationQuestions.",
            if (batchMode) {
            ...
            val fields = JSONArray(
                listOf(
                    ...
                    JSONObject().put("id", "mechanicalItems").put("key", "mechanicalItems").put("label", "Strojarski dio")
                        .put("instructions", "Ako je context.recognitionMode='template', vrati mechanicalItems=[] jer Android strojarske stavke uzima iskljucivo iz predloska. U detaljnom nacinu vrati relevantne strojarske stavke za ovaj stroj odmah iz slike cijelog stroja, komandi, zastita, pokretnih dijelova, stabilnosti i plocice. Ciljaj najmanje 12 relevantnih stavki kada fotografije daju dovoljno konteksta. Ako bi napisao treba provjeriti/potvrditi, vrati pitanje u verificationQuestions umjesto stavke."),
                    JSONObject().put("id", "electricalItems").put("key", "electricalItems").put("label", "Elektro dio")
                        .put("instructions", "Ako je context.recognitionMode='template', vrati electricalItems=[] jer Android elektro stavke uzima iskljucivo iz predloska. U detaljnom nacinu, ako oprema ima kabel, utikac, napajanje, elektromotor, upravljacku elektroniku, sonde ili plocicu s U/F/P podacima, vrati relevantne elektro stavke."),
                    JSONObject().put("id", "riskRegisterIris").put("key", "hazardRegisterIris").put("label", "Opasnosti, stetnosti i napori")
                        .put("instructions", "Ako je context.recognitionMode='template', vrati hazardRegisterIris=[], harmfulnessRegisterIris=[] i strainRegisterIris=[] jer Android rizike uzima iskljucivo iz predloska. U detaljnom nacinu vrati sigurne IRI-jeve iz slike i plocice."),
                ),
            )
            val body = JSONObject()
            ...
            val fields = JSONArray(
                listOf(
                    ...
                    JSONObject().put("id", "measuresToEliminateDeficiencies").put("key", "measuresToEliminateDeficiencies").put("label", "Mjere")
                        .put("instructions", "Ako nema nedostataka, vrati 'Nisu potrebne posebne mjere prema dostavljenim slikama.' Ako je nesto nesigurno, preporuci dodatnu provjeru ili dokumentiranje. Ostro formuliraj samo jasno vidljive nedostatke."),
                    JSONObject().put("id", "mechanicalItems").put("key", "mechanicalItems").put("label", "Strojarski dio")
                        .put("instructions", "Ako je context.recognitionMode='template', vrati mechanicalItems=[] jer Android strojarske stavke uzima iskljucivo iz predloska. U detaljnom nacinu za svaku prepoznatu opremu vrati relevantne strojarske stavke odmah iz slike cijelog stroja, komandi, zastita, pokretnih dijelova, stabilnosti i plocice."),
                    JSONObject().put("id", "electricalItems").put("key", "electricalItems").put("label", "Elektro dio")
                        .put("instructions", "Ako je context.recognitionMode='template', vrati electricalItems=[] jer Android elektro stavke uzima iskljucivo iz predloska. U detaljnom nacinu, ako oprema ima kabel, utikac, napajanje, elektromotor, upravljacku elektroniku, sonde ili plocicu s U/F/P podacima, vrati relevantne elektro stavke."),
                    JSONObject().put("id", "riskRegisterIris").put("key", "hazardRegisterIris").put("label", "Opasnosti, stetnosti i napori")
                        .put("instructions", "Ako je context.recognitionMode='template', vrati hazardRegisterIris=[], harmfulnessRegisterIris=[] i strainRegisterIris=[] jer Android rizike uzima iskljucivo iz predloska. U detaljnom nacinu vrati sigurne IRI-jeve iz slike i plocice."),
